...
$loan_q = mysql_query("SELECT * FROM cashbook WHERE package_id = '".$package_id."' AND date LIKE '%".$month."%' ORDER BY date ASC");

$prevbal_q = mysql_query("SELECT SUM(amount) AS amtOut FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'LOAN' AND date < '".$month."'");
$get_pb = mysql_fetch_assoc($prevbal_q);

$prevbal2_q = mysql_query("SELECT SUM(amount) AS amtIn FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'REC' AND date < '".$month."'");
$get_pb2 = mysql_fetch_assoc($prevbal2_q);

$prevbal3_q = mysql_query("SELECT SUM(amount) AS amtEx FROM cashbook WHERE package_id = '".$package_id."' AND type = 'EXPENSES' AND date < '".$month."'");
$get_pb3 = mysql_fetch_assoc($prevbal3_q);

$prevbal4_q = mysql_query("SELECT SUM(amount) AS amtKom FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'KOMISYEN' AND date < '".$month."'");
$get_pb4 = mysql_fetch_assoc($prevbal4_q);

$prevbal5_q = mysql_query("SELECT SUM(amount) AS amtOth FROM cashbook WHERE package_id = '".$package_id."' AND type = 'RECEIVED2' AND date < '".$month."'");
$get_pb5 = mysql_fetch_assoc($prevbal5_q);

$prevbal6_q = mysql_query("SELECT SUM(amount) AS amtKom2 FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'KOMISYEN2' AND date < '".$month."'");
$get_pb6 = mysql_fetch_assoc($prevbal6_q);

$prevbal7_q = mysql_query("SELECT SUM(amount) AS amtCCM FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'CCM' AND date < '".$month."'");
$get_pb7 = mysql_fetch_assoc($prevbal7_q);

$prevbal8_q = mysql_query("SELECT SUM(amount) AS amtTrans FROM cashbook WHERE package_id = '".$package_id."' AND type = 'TRANSFER1' AND date < '".$month."'");
$get_pb8 = mysql_fetch_assoc($prevbal8_q);

$prevbal9_q = mysql_query("SELECT SUM(amount) AS amtTrans2 FROM cashbook WHERE package_id = '".$package_id."' AND type = 'TRANSFER2' AND date < '".$month."'");
$get_pb9 = mysql_fetch_assoc($prevbal9_q);

$prevbal10_q = mysql_query("SELECT SUM(amount) AS amtExt FROM cashbook WHERE package_id = '".$package_id."' AND transaction = 'EXT' AND date < '".$month."'");
$get_pb10 = mysql_fetch_assoc($prevbal10_q);

$cf = $cf - $get_pb['amtOut'] + $get_pb2['amtIn'] - $get_pb3['amtEx'] + $get_pb4['amtKom'] + $get_pb5['amtOth'] + $get_pb6['amtKom2'] + $get_pb7['amtCCM'] - $get_pb8['amtTrans'] + $get_pb9['amtTrans2'] - $get_pb10['amtExt'];
...
<table width="1280" id="list_table">
	<tr>
    	<td colspan="2"><strong>CASH BOOK <?php echo $get_p['scheme']." (".date('M Y', strtotime($nmth)).")"; ?></strong></td>
    </tr>
	<tr>
    ...
    <?php } ?>
	<tr>
    	<?php if($get_p['type'] != 'Fixed Loan'){ if($get_p['receipt_type'] == '1') {?><td colspan="6">&nbsp;</td><?php } else { ?>
		<td colspan="5">&nbsp;</td>
		<?php } } else { ?>
		<td colspan="4">&nbsp;</td>
		<?php } ?>
    ...
    <tr>
		...
		<?php } } else { ?>
		<td colspan="5" align="right"><input type="button" name="back" id="back" onClick="window.location.href='../cashbook/cashbook.php?id=<?php echo $package_id; ?>'" value=""></td>
		<?php } ?>
    </tr>
    <tr>
    	<td colspan="2">&nbsp;</td>
