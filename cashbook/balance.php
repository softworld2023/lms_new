...
<table width="1280" id="list_table">
    ...
    <?php 
	$ctr = 0;
	$package_q = mysql_query("SELECT * FROM loan_scheme WHERE scheme != 'SKIM KUTU' AND scheme != 'KUTU OFFICE' ORDER BY scheme ASC");
    while($get_p = mysql_fetch_assoc($package_q)){
	$ctr++;
	
	$bal2cf_q = mysql_query("SELECT * FROM bal2_cf WHERE package_id = '".$get_p['id']."' AND branch_id = '".$_SESSION['login_branchid']."' AND month = ''");
	$bal2cf = mysql_fetch_assoc($bal2cf_q);
	?>
    <tr>
    	<td><?php echo $ctr."."; ?></td>
        <td><a href="balance2.php?id=<?php echo $get_p['id']; ?>"><?php echo $get_p['scheme']; ?></a></td>
        <td><?php echo "RM ".number_format($bal2cf['amount'], '0'); ?></td>
        <td><?php if($bal2cf['date'] != '0000-00-00') { echo date('d-m-Y', strtotime($bal2cf['date'])); } else { echo "-"; } ?></td>
        <td>
            ...
            <table>
            	<tr>
                    ...
                    <td style="padding:0px">
                   	  <input type="text" name="amount_<?php echo $ctr; ?>" id="amount_<?php echo $ctr; ?>" value="<?php echo $bal2cf['amount']; ?>" class="currency">                    </td>
                    <td>Edit Date</td>
                    <td style="padding:0px">
                   	  <input type="text" name="opening_<?php echo $ctr; ?>" id="opening_<?php echo $ctr; ?>" value="<?php if($bal2cf['date'] != '0000-00-00') { echo $bal2cf['date']; }?>">                    </td>
                    <td>
