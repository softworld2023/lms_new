...
if(isset($_POST['search']))
{
	...
	$statement = "`expenses` WHERE branch_id = '".$_SESSION['login_branchid']."' $cond AND display_status = 'SHOW' ORDER BY date ASC, id ASC";
	
}

else
{
	$statement = "`expenses` WHERE branch_id = '".$_SESSION['login_branchid']."' AND month_receipt = '".date("Y-m")."' AND display_status = 'SHOW' ORDER BY date ASC, id ASC";
}
...
//$query = mysql_query("SELECT * FROM expenses ORDER BY date DESC");
?>
...
<table width="1280" id="list_table">
	...
	while($get_q = mysql_fetch_assoc($query)){ 
...
if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'bida' || $_SESSION['login_username'] == 'huizhen'))
		...
		?>
		<td>
			<a href="edit_expenses.php?id=<?php echo $get_q['id']; ?>"><img src="../img/customers/edit-icon.png" /></a>&nbsp;&nbsp;<a href="javascript:deleteConfirmation('<?php echo $get_q['expenses_details']; ?>', '<?php echo $get_q['id']; ?>')"><img src="../img/customers/delete-icon.png" title="Delete"></a>
		</td>
