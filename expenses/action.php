...
if(isset($_POST['add_expenses']))
{
	...
	//insert expenses
	$insert_q = mysql_query("INSERT INTO expenses SET 
		...
		$insert_q_acc = mysql_query("INSERT INTO expenses_accountant SET 
								expenses_type = '".$details1."', 
								expenses_details = '".$details2."', 
								date = '".$date."', 
								amount = '".$amount."', 
...
if(isset($_POST['edit_expenses']))
{
	...
	//get prev rec
	$pexp_q = mysql_query("SELECT * FROM expenses WHERE id = '".$id."'");
	$pexp = mysql_fetch_assoc($pexp_q);
	
	//get prev rec for expenses account
	$ex_a_q = mysql_query("SELECT * FROM expenses_accountant WHERE expenses_type='".$pexp['expenses_type']."' 
	AND expenses_details='".$pexp['expenses_details']."'
	AND amount = '".$pexp['amount']."' 
	AND display_status ='SHOW' ");
	$ex_a = mysql_fetch_assoc($ex_a_q);
	
	//update expenses
	$insert_q = mysql_query("UPDATE expenses SET expenses_type = '".$details1."', expenses_details = '".$details2."', date = '".$date."', amount = '".$amount."', package_id = '".$package_id."', ttype = '".$ttype."', month_receipt = '".$month_receipt."' WHERE id = '".$id."'");
	
	//update expenses account
	
	$insert_q = mysql_query("UPDATE expenses_accountant SET expenses_type = '".$details1."', 
									expenses_details = '".$details2."', 
									date = '".$date."', 
									amount = '".$amount."', 
									package_id = '".$package_id."', 
...
if($_POST['action'] == 'delete_expenses')
{
	...
	$amount = $pexp['amount'];
	$date = $pexp['date'];
	$details1 = mysql_real_escape_string($pexp['expenses_type']);
	$details2 = mysql_real_escape_string($pexp['expenses_details']);
	...
	$sql = "UPDATE cashbook SET display_status = 'DELETED' WHERE package_id = '".$package_id."' AND type = '".$type."' AND transaction = '".$expenses_details."' AND table_id = '".$id."' AND amount = '".$amount."' AND date LIKE '%".$date."%' AND branch_id = '".$branch_id."'";
	// var_dump($sql);
	// exit;
	$deletecb = mysql_query($sql);
	if($deletecb)
	{
		...
		$delete_ac = mysql_query("UPDATE expenses SET display_status = 'DELETED' WHERE id = '".$id."'");
