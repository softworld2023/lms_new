					...
					<td>
						<table id="list_table" width="350">
							...
							$person_q = mysql_query("SELECT * FROM cashbook WHERE type = 'PAY' AND date LIKE '%".$_SESSION['statementmonth']."%' AND branch_id = '".$_SESSION['login_branchid']."' AND package_id = '".$package_id."'  AND receipt_no LIKE '".$m."%' ORDER BY receipt_no DESC");
								$person1 = mysql_fetch_assoc($person_q);
								if(substr($person1['receipt_no'], -3, -2) != '0')
								{
									$person = substr($person1['receipt_no'], -3);
								} else
								{
									$person = substr($person1['receipt_no'], -2);
								}
							
								if(mysql_num_rows($person_q) == 0)
								{
									$person = '0';				
								}
				...
				<tr>
					<td>
						<table id="list_table" width="350">
							...
							// new script on 05/07/2016
							/*$person_q = mysql_query("SELECT * FROM cashbook WHERE date LIKE '%".$_SESSION['statementmonth']."%' AND branch_id = '".$_SESSION['login_branchid']."' AND package_id = '".$package_id."'  AND receipt_no LIKE '".$m."%' ORDER BY receipt_no DESC");
								$person1 = mysql_fetch_assoc($person_q);
								if(substr($person1['receipt_no'], -3, -2) != '0')
								{
									$person = substr($person1['receipt_no'], -3);
