...
if(isset($_POST['search']))
{
	...
	$statement = "`cashbook` WHERE package_id = '".$id."' AND type = 'PAY' AND date LIKE '%".$date."%'";
	$_SESSION['dailyloan'] = $statement;
}
else
if($_SESSION['dailyloan'] != '')
{
	$statement = $_SESSION['dailyloan'];
}
else
{
	$statement = "`cashbook` WHERE package_id = '".$id."' AND type = 'PAY' AND date LIKE '%".$date."%'";
...
<table width="1280">
    ...
    <tr>
    	<td colspan="3">
        <div class="subnav">
			<a href="index.php">Borrow Out</a><a href="payout.php">Actual Payout</a><a href="collection.php">Total Collection</a><a href="profit.php">Profit & Loss</a><a href="expenses.php">Expenses</a><a href="interest.php">Interest Earn</a><a href="latepayment.php">Late Payment Collections</a><a href="daily.php">Daily Collections</a><a href="statement.php">Statement</a></div>        </td>
    </tr>
</table>
...
<table width="1280" id="list_table">
	...
	while($get_q = mysql_fetch_assoc($sql))
	{
		...
		$payment = $get_q['amount'];
		$total += $payment;
		$ctr++;
	?>
    <tr>
    	<td><?php echo $ctr."."; ?></td>
        <td><?php echo date('Y-m-d', strtotime($get_q['date'])); ?></td>
