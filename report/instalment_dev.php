...
$payout_month = date("Y-m");
 ...
 <?php 
    ...
    <tr style="<?php echo $style.";"; ?>">
		...
		<td align="center" style="border-bottom: 1px dashed black;"><a style="<?php echo $style.";"; ?>" href="payloan_a.php?id=<?php echo $result_4['customer_loanid']; ?>&year=<?php echo $instalment_year; ?>&month=<?php echo $instalment_month; ?>" title="Make Payment">
		<?php if(substr($result_4['loan_code'],0,2) == 'MS' 
						...
						|| substr($result_4['loan_code'],0,2) == 'YW'){echo preg_replace('/^([a-z]{2})/i','<span style="color:red;">\1</span>',$result_4['loan_code']);}else { echo $result_4['loan_code'];} ?></a></td>
...
else
{
	$style_bd = 'background-color:yellow;';
							...
							if ($result_4['loan_code'] == 'KT 24015') {
								$month_receipt_parts = explode('-', $payout_month);
								$year_in_context = $month_receipt_parts[0];
								$month_in_context = $month_receipt_parts[1];
								$month_receipt_in_context = date('Y') . '-' . $month_in_context;
								$get_previous_months_bd_collected_sql = "SELECT SUM(amount) AS collected FROM late_interest_payment_details WHERE lid = '" . $result_4['id'] . "' AND month_receipt >= '$year_in_context-01' AND month_receipt < '" . $month_receipt_in_context . "'";
								var_dump($get_previous_months_bd_collected_sql);
								$query = mysql_query($get_previous_months_bd_collected_sql);
								while ($row = mysql_fetch_assoc($query)) {
...
?>			
	...
	<tr ><td style = "text-align:center;">&nbsp;</td><td style = "text-align:center;">&nbsp;</td><td style = "text-align:center;"><?php echo $ctr2;?></td>
			...
			|| substr($result_5['loan_code'],0,2) == 'YW'){echo preg_replace('/^([a-z]{2})/i','<span style="color:red;">\1</span>',$result_5['loan_code']);}else { echo $result_5['loan_code'];} ?></td>
