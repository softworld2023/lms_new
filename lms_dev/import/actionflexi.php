...
do
{
	...
	$interest = $objPHPExcel->getActiveSheet()->getCell("I$i")->getValue();
	$package = $objPHPExcel->getActiveSheet()->getCell("J$i")->getValue();
	$lperiod = $objPHPExcel->getActiveSheet()->getCell("K$i")->getValue();
	$ltype = $objPHPExcel->getActiveSheet()->getCell("L$i")->getValue();
	$balance = $objPHPExcel->getActiveSheet()->getCell("M$i")->getValue();
	$npd = $objPHPExcel->getActiveSheet()->getCell("N$i")->getValue();
	$month_receipt = $objPHPExcel->getActiveSheet()->getCell("O$i")->getValue();
	
	$bank = $objPHPExcel->getActiveSheet()->getCell("P$i")->getValue();
	$paydate = $objPHPExcel->getActiveSheet()->getCell("Q$i")->getValue();
	$loan_remarks = $objPHPExcel->getActiveSheet()->getCell("R$i")->getValue();
	$intper = $objPHPExcel->getActiveSheet()->getCell("S$i")->getValue();
	...
	}else
	{
		if($nric != '')
		{
			...
			$customer_acc = mysql_query("INSERT INTO customer_account SET customer_id = '".$cust_id."', a_payday = '".$paydate."', a_bankbranch = '".$bank."'");
	...
	$insert_clp = mysql_query("INSERT INTO customer_loanpackage SET customer_id = '".$cust_id."', loan_code = '".$code."', loan_package = '".mysql_real_escape_string($package)."', loan_total = '".$total_loan."', loan_amount = '".$loan_amount."', loan_period = '".$lperiod."', loan_status = 'Paid', loan_type = '".$ltype."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."', loan_remarks = '".$loan_remarks."'");		
	...
	if($insert_clp)
	{
		...
		//skim f
		$insert_payment = mysql_query("INSERT INTO loan_payment_details SET customer_loanid = '".$lp_id."', int_percent = '".$intper."', month = '1', int_balance = '".$interest."', balance = '".$balance."', package_id = '".$get_pack['id']."', next_paymentdate = '".$npd."', receipt_no = '".$code."', month_receipt = '".$month_receipt."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."'");
		
		//bukan skim f
		//$insert_payment = mysql_query("INSERT INTO loan_payment_details SET customer_loanid = '".$lp_id."', int_percent = '".$get_pack['interest']."', month = '1', int_balance = '".$interest."', balance = '".$balance."', package_id = '".$get_pack['id']."', next_paymentdate = '".$npd."', receipt_no = '".$code."', month_receipt = '".$month_receipt."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."'");
	}
	//sql end
	$i++;
