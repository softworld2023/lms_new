...
$sql = mysql_query("SELECT * FROM expenses WHERE id = '".$id."'");
...
<center>
<form method="post" action="action.php" onSubmit="return checkForm()"><input type="hidden" name="id" id="id" value="<?php echo $id; ?>">
<table width="1280">
	<tr>
...
<table width="1280" id="list_table">
	...
	<tr>
        ...
        <td><input type="text" name="expenses_details" id="expenses_details" value="<?php echo $get_q['expenses_details'] ?>" style="width:350px" /></td>
    </tr>
    <tr>
    	<th>Date</th>
    ...
    <tr>
	  ...
	  <select name="package_id" id="package_id" style="height:30px;">
        <option value=""></option>
        <?php 
			$package_q = mysql_query("SELECT * FROM loan_scheme WHERE type = 'Flexi Loan'");
			while($get_p = mysql_fetch_assoc($package_q))
			{
		?>
        <option value="<?php echo $get_p['id']; ?>" <?php if($get_q['package_id'] == $get_p['id']) { echo 'selected'; } ?>><?php echo $get_p['scheme']; ?></option>
        <?php 
			}
		?>
      </select>
