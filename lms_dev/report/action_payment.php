...
if($_POST['action'] == 'payloan')
{
	...
	$period = $_POST['period'];
	$month = $_POST['month'];
	$ori_instalment = $_POST['ori_instalment'];
	$next_paymentdate = date('Y-m-d', strtotime($_POST['nextdate']));
	$receipt_no = $_POST['receipt'];
	...
	if($ar == 0)
	{
		...
		if($update_q)
		{
			...
			$nextp = mysql_fetch_assoc($nextp_q);
							...
							// var_dump('2');
							// var_dump("SELECT * FROM loan_payment_details WHERE 
							// 		customer_loanid = '".$get_q['customer_loanid']."' 
							// 		AND id > '".$id."' ");

			$newmonth = $month + 1;
			// if(mysql_num_rows($nextp_q)>0){
			if(!$nextp){
				//if($newmonth <= $period)
				//{
							// var_dump('2');
					...
					{	
						...
						// var_dump("INSERT INTO loan_payment_details SET 
						// 							customer_loanid = '".$get_q['customer_loanid']."', 
						// 							month = '".$newmonth."', 
						// 							int_percent = '".$get_q['int_percent']."', 
						// 							balance = '".$newbalance."', 
		...
		if($update_q)
		{
			...
			if($newmonth <= $period)
			{
				...
				if($newbalance > 0)
				{
					...
					$update_lp = mysql_query("UPDATE half_payment_details SET 
													loan_status = 'Completed' 
													WHERE customer_id = '".$get_lp['customer_id']."' AND
													loan_code = '".$get_lp['loan_code']."' 
													");		
...
if($_POST['action'] == 'delete_payout')
{
	...
	$custid = $_POST['custid'];
		...
		$sql = mysql_query("DELETE monthly_payment_record_draft WHERE customer_id = '".$custid."'");
