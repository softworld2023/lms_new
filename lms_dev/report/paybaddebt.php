...
$query = mysql_query("SELECT SUM(balance) AS previous_monthly_bd FROM monthly_payment_record WHERE loan_code = '" . $get_q['loan_code'] . "' AND status = 'BAD DEBT'");
...
} else if ($branch_name != 'majusama' && $bd_from == 'Instalment + Monthly') {
	$amount -= $previous_monthly_bd;
}
...
$payment = mysql_query("SELECT * FROM late_interest_payment_details WHERE lid = '".$id."'");
?>
