...
<center>
							...
							<td style="font-size: 16px;"><br>YEAR
								...
								<select id="instalment_month" name="instalment_month" style="width: 120px;height: 30px; font-size:16px;">
									...
									?>
										...
										<option value='<?php echo $i; ?>' <?php echo $selected; ?>><?php echo $full_month_name; ?></option>
									<?php
									}
									?>
	...
	<table width="1280" id="list_table">
		<tr style="background-color: #45b1e8;">
			<td colspan='9' style="color:black;font-size: 22px;"><b>
				<?php if ($instalment_month != '') {
					echo 'Instalment ' . $mth1 . ' - ' . $instalment_year;
				} else {
					echo 'Instalment ' . date("M - Y");
				} ?><b>
	...
	<table width="1280" id="list_table">
		...
		$str = "SELECT
			...
			FROM
				...
				LEFT JOIN late_interest_record ON late_interest_record.loan_code = customer_loanpackage.loan_code
			WHERE
				temporary_payment_details.loan_month = '" . $payout_month . "'
			AND 
				customer_details.name != 'UNREGISTERED'
			AND
				customer_loanpackage.loan_code NOT IN
				(SELECT
					customer_loanpackage.loan_code
				FROM
					customer_loanpackage
				LEFT JOIN
					late_interest_record ON late_interest_record.loan_code = customer_loanpackage.loan_code
				WHERE
					late_interest_record.month_receipt < '" . $payout_month . "'
				)
			AND 
				temporary_payment_details.loan_code NOT IN
				(SELECT
					customer_loanpackage.loan_code 
				FROM
					customer_loanpackage
				...
				AND 
					loan_payment_details.loan_status='SETTLE' 
			...
			AND 
				...
				AND
					loan_payment_details.month_receipt <'" . $payout_month . "'
				AND 
					loan_payment_details.loan_status='BAD DEBT' 
				GROUP BY customer_loanpackage.loan_code
				ORDER BY customer_loanpackage.loan_code ASC 
				)
			GROUP BY customer_loanpackage.loan_code
			ORDER BY customer_loanpackage.start_month = '" . $payout_month . "', customer_loanpackage.payout_date ASC";
		// var_dump("1. " . $str);
		$sql_4 = mysql_query($str);
    ...
    <tr style="<?php echo $style.";"; ?>">
    	<td style="border-left: 1px solid black;border-bottom: 1px dashed black;"><?php echo $ctr."."; ?></td>
				<td align="center" style="border-bottom: 1px dashed black;">
			<a style="<?php echo $style.";"; ?>" href="payloan_a.php?id=<?php echo $result_4['customer_loanid']; ?>&year=<?php echo $instalment_year; ?>&month=<?php echo $instalment_month; ?>" title="Make Payment">
						<?php 
				if(substr($result_4['loan_code'],0,2) == 'MS' 
				|| substr($result_4['loan_code'],0,2) == 'MJ' 
				|| substr($result_4['loan_code'],0,2) == 'PP' 
				|| substr($result_4['loan_code'],0,2) == 'SB' 
				|| substr($result_4['loan_code'],0,2) == 'TL' 
				|| substr($result_4['loan_code'],0,2) == 'YW' 
				|| substr($result_4['loan_code'],0,2) == 'CL' 
				|| substr($result_4['loan_code'],0,2) == 'BT')
					{echo preg_replace('/^([a-z]{2})/i','<span style="color:red;">\1</span>',$result_4['loan_code']);
				}else { 
																																																																							echo $result_4['loan_code'];
				} ?>
			</a>
		</td>
				...
				<td style="border-right: 1px dashed black;border-bottom: 1px dashed black;text-align: center;
				<?php if ($out != '') {echo 'color:red;';} else {}; ?>"><?php echo number_format($out); ?></td>		
				<td style="border-right: 1px solid black;border-bottom: 1px dashed black;text-align: center;
				<?php 
				if ($collected_remarks == 'SETTLE') {
					echo 'background-color: yellow;';
				} elseif ($collected_remarks == 'BD') {
					echo 'background-color: darkorange;';
				} elseif ($last_loanremarks == 'LAST') {
					echo 'background-color: yellow;';
				} else {
				} ?>"><?php echo number_format($collected) . '' . $collected_remarks; ?></td>
	...
	<?php
			$loginBranch = $_SESSION["login_branch"];

			// if ($loginBranch == "ANSENG"){
			...
			while ($result_bd_collected = mysql_fetch_assoc($sql_bd_collected)) {
				if ($loginBranch == 'ANSENG' && $payout_month == '2023-04') {
					$totalamount_collected_desc += 0;
				} else {
	...
	<div style="font-size:10px;">
		<table width="65%" align="left" id="list_table" cellspacing="1" cellpadding="2" style="font-family:Time New Roman;padding-right:150px;padding-left:50px;padding-bottom:-230px;font-size:12px;color:black ;">
			...
			while ($result_4 = mysql_fetch_assoc($sql_4)) {
				...
				$sql_4a = mysql_query("SELECT
										...
										late_interest_record.amount AS amount,
										SUM(
											late_interest_payment_details.amount
										) AS collected,
										MAX(
											late_interest_payment_details.payment_date
										) AS payment_date,
									...
									LEFT JOIN customer_details ON late_interest_record.customer_id = customer_details.id
									LEFT JOIN late_interest_payment_details ON late_interest_record.id = late_interest_payment_details.lid
									LEFT JOIN collection ON collection.loan_code = late_interest_record.loan_code
									AND collection.instalment_month = '" . $payout_month . "'
									WHERE
										late_interest_record.month_receipt <= '" . $payout_month . "'
									AND (
										late_interest_payment_details.collection_status IS NULL
										OR late_interest_payment_details.collection_status != 'PENDING'
									)
				...
				// $payment_date = date("d/M/Y", strtotime($result_4['payment_date']));
				$amount_collected = 0;
				...
				$query = mysql_query("SELECT SUM(payout_amount) AS previous_monthly_amount, SUM(balance) AS previous_monthly_bd FROM monthly_payment_record WHERE loan_code = '" . $result_4['loan_code'] . "' AND status = 'BAD DEBT'");
				...
				if ($branch_name == 'majusama_dev' && $bd_from == 'Instalment + Monthly' && $late_interest_record_id > 46) {
					$amount -= $previous_monthly_bd;
				}
				// else if ($branch_name == 'yuwang') {
				...
				// 	}
				// } 
				else if ($branch_name != 'majusama_dev' && ($bd_from == 'Instalment + Monthly' || $bd_from == 'Monthly')) {
					$amount -= $previous_monthly_bd;
				}
				...
				// var_dump($result_4['id']);

				$query = mysql_query("SELECT SUM(amount) AS collected, month_receipt 
										FROM late_interest_payment_details 
										WHERE (collection_status != 'PENDING' OR collection_status IS NULL )
										AND lid = '" . $result_4['id'] . "' 
				...
				// if ($branch_name == 'majusama_dev' && $late_interest_record_id > 46) {
				// 	$amount -= $current_month_bd_collected;
				// }
				...
				if ($previous_monthly_bd == 0 && $result_4['bd_from'] != 'Monthly') {
					$envelope -= $deductible;
				}
				...
				if ($branch_name == 'majusama_dev' && $late_interest_record_id <= 46) {
					$previous_monthly_bd = 0;
					$envelope = 0;
				} else if ($branch_name == 'yuwang' && $result_4['loan_code'] == 'YW1210') {
					$envelope = 0;
				}
				...
				// if ($branch_name == 'majusama_dev' && $late_interest_record_id <= 46) {
				// 	$balance -= $current_month_bd_collected;
				// }

				$selected_year_month = $instalment_year . '-' . date('m', mktime(0, 0, 0, $instalment_month));
				// var_dump($selected_year_month);
				// var_dump($current_month_receipt);

				if ($result_4['loan_code'] != 'YW1210') {
					if (($balance == 0 || ($bd_from == 'Monthly' && $previous_monthly_bd == 0)) && $selected_year_month > $current_month_receipt) {
						continue;	// not showing this row if balance is zero, but still showing if the balance becomes zero in the current month, and it's removed only on the next month
					}
				} else {
					if (($balance == 0 || ($bd_from == 'Monthly' && $previous_monthly_bd == 0)) && !empty($current_month_receipt) && $selected_year_month > $current_month_receipt) {
						continue;	// not showing this row if balance is zero, but still showing if the balance becomes zero in the current month, and it's removed only on the next month
					}
				}
			...
			<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
				...
				<td width="10%" style = "text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;<?php echo $style_bd;?>"><a href="../payment/payLateInt.php?id=<?php echo $result_4['id'];?>" title="Make Payment">
					<?php if(substr($result_4['loan_code'],0,2) == 'MS' 
					|| substr($result_4['loan_code'],0,2) == 'MJ' 
					|| substr($result_4['loan_code'],0,2) == 'PP' 
					|| substr($result_4['loan_code'],0,2) == 'SB' 
					|| substr($result_4['loan_code'],0,2) == 'TL' 
					|| substr($result_4['loan_code'],0,2) == 'YW'){echo preg_replace('/^([a-z]{2})/i','<span style="color:red;">\1</span>',$result_4['loan_code']);}else { echo $result_4['loan_code'];} ?></a></a></td>
			...
			<tr>
				...
				<td style="text-align:center;"></td>
				<td style="text-align:center;"></td>
				<td colspan="8" style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;"><b>Bad Debts This Month (Instalment)</b></td>
			</tr>
			<tr>
			<tr>
				...
				<td colspan="2" width="5%" style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;">AMOUNT(INSTALMENT)</td>
			</tr>
			<?php
			$ctr2 = 0;
			...
			?>
				...
				<tr>
					<td style="text-align:center;">&nbsp;</td>
					<td style="text-align:center;">&nbsp;</td>
					<td style="text-align:center;"><?php echo $ctr2; ?></td>
					<td style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;background-color:orange;">
						<?php if (substr($result_5['loan_code'], 0, 2) == 'MS' 
						|| substr($result_5['loan_code'], 0, 2) == 'MJ' 
						|| substr($result_5['loan_code'],0, 2) == 'PP' 
						|| substr($result_5['loan_code'], 0, 2) == 'SB' 
						|| substr($result_5['loan_code'], 0, 2) == 'TL' 
						|| substr($result_5['loan_code'], 0, 2) == 'YW') {
							echo preg_replace('/^([a-z]{2})/i', '<span style="color:red;">\1</span>', $result_5['loan_code']);
						} else {
							echo $result_5['loan_code'];
						} ?></td>
					...
					<td colspan="2" style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;background-color:orange;"><?php echo $result_5['amount']; ?></td>
				</tr><?php } ?>
			...
			<tr>
				...
				<td style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;"></td>
				<td style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;"></td>
				<td colspan="2" style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;"></td>
			</tr>
			<tr>
				<td style="text-align:center;"></td>
				<td style="text-align:center;"></td>
				<td style="text-align:center;"></td>
				<td colspan="6" style="text-align:right;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;">TOTAL</td>
				<td colspan="2" style="text-align:center;border-top:1px solid;border-right:1px solid;border-left:1px solid;border-bottom: 1px solid;"><?php echo number_format($totalamount_instalment); ?></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
			</tr>
        ...
        <td>&nbsp;</td> -->
			<td colspan="3" style="color:#FF00FF;"><input type="text" name="opening_balance_k" id="opening_balance_k" value=" <?php echo number_format($opening_balance_k, 2); ?>" />
				<?php
			...
			<tr>
				...
				<td>&nbsp;</td>
				<!-- <td colspan="3" style="color:#FF00FF;"><input type="text" name="collected_k" id="collected_k" value=" <?php echo number_format($collected_k, 2); ?>" />
		<?php
		// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
		?>
		...
		?>
			...
			<tr>
				...
				<td>&nbsp;</td>
				<!-- <td colspan="3" style="color:#FF00FF;"><input type="text" name="settle_k" id="settle_k" value=" <?php echo number_format($settle_k, 2); ?>" />
		<?php
		// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
		?>
		...
		?>
			...
			<tr>
				...
				<td colspan="3" style="color:#FF00FF;"><input type="text" name="capital_in" id="capital_in" value=" <?php echo number_format($capital_in, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
			...
			<tr>
				...
				<td>&nbsp;</td>
				<!--         <td colspan="3" style="color:#FF00FF;"><input type="text" name="baddebt_k" id="baddebt_k" value=" <?php echo number_format($baddebt_k, 2); ?>" />
		<?php
		// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
		?>
        ...
        <td>&nbsp;</td> -->
				<td colspan="3" style="color:#FF00FF;"><input type="text" name="monthly_k" id="monthly_k" value=" <?php echo number_format($monthly_k, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
			...
			<tr>
				...
				<td>&nbsp;</td>
				<!-- <td colspan="3" style="color:#FF00FF;"><input type="text" name="payout_k" id="payout_k" value=" <?php echo number_format($payout_k, 2); ?>" />
		<?php
		// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
		?>
        ...
        <td>&nbsp;</td> -->
				<td colspan="3" style="color:#FF00FF;"><input type="text" name="expenses_k" id="expenses_k" value=" <?php echo number_format($expenses_k, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
			...
			<tr>
				...
				<td colspan="3" style="color:#FF00FF;"><input type="text" name="expenses_k2" id="expenses_k2" value=" <?php echo number_format($expenses_k2, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
			...
			<tr>
				...
				<td colspan="3" style="color:red;"><input type="text" name="interest_paid_out" id="interest_paid_out" value=" <?php echo number_format($interest_paid_out, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
			...
			<tr>
				...
				<td colspan="3" style="color:red;"><input type="text" name="return_capital" id="return_capital" value=" <?php echo number_format($return_capital, 2); ?>" />
					<?php
					// if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { 
					?>
