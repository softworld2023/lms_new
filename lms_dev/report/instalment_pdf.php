	...
	while ($result_bf = mysql_fetch_assoc($sql_balance_bf)) { 
		...
		while ($row_bf = mysql_fetch_assoc($sql)) {
			if ($result_bf['loan_code'] == $row_bf['receipt_no']) {
				if ($result_bf['loan_period'] >= 1 && $result_bf['loan_period'] <= 12) {
				...
				} else if ($result_bf['loan_period'] >= 13 && $result_bf['loan_period'] <= 24) {
					if ($result_bf['loan_status'] == 'SETTLE') {
						$collected_bf = '';
						$settle_bf = $result_bf['loan_percent'];
					} else if ($row_bf['loan_status'] == 'BAD DEBT') {
						$collected_bf = '';
				...
				} else if ($result_bf['loan_period'] >= 25 && $result_bf['loan_period'] <= 36) {
					if ($row_bf['loan_status'] == 'SETTLE') {
						$collected_bf = '';
						$settle_bf = $result_bf['loan_percent'];
					} else if ($row_bf['loan_status'] == 'BAD DEBT') {
				...
				} else if ($result_bf['loan_period'] >= 37 && $result_bf['loan_period'] <= 48) {
					if ($row_bf['loan_status'] == 'SETTLE') {
						$collected_bf = '';
						$settle_bf = $result_bf['loan_percent'];
					} else if ($row_bf['loan_status'] == 'BAD DEBT') {
		...
		$closing_balance = $opening_balance_k + $totalcollected_bf + $totalsettle_bf + $capital_in_bf + $baddebt_bf + $monthly_bf - $totalout_bf - $expenses_bf - $expenses_bf2 - $interest_paid_out_bf - $return_capital_bf;
...
$html.='<table width="25%" border="0" cellspacing="0" cellpadding="2" align="right" style="font-family:Time New Roman;padding-right:20px;padding-left:50px;padding-bottom:-230px;padding-top:30px;font-size:10px;">	
...
$html.='			
			<tr>
					...
					<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;">'.number_format($total_envelope).'</td></tr>
				</table></div><div>
			<div style="font-size:8px;">
...
$html.='	<tr ><td style = "text-align:center;">'.$ctr2_list.'</td>
		...
		';
			if(substr($result_5_list['loan_code'],0,2) =='MS' || substr($result_5_list['loan_code'],0,2) =='MJ' || substr($result_5_list['loan_code'],0,2) =='SB' || substr($result_5_list['loan_code'],0,2) =='YW')
						{
							$html.='
			<td style = "text-align:center;border-right:1px solid black;border-left:1px solid black;border-bottom: 1px solid black;background-color:orange;">'.preg_replace('/^([a-z]{2})/i','<span style="color:red;">\1</span>',$result_5_list['loan_code']).'</td>';
		}else
		{
			$html.='
			<td style = "text-align:center;border-right:1px solid black;border-left:1px solid black;border-bottom: 1px solid black;background-color:orange;">'.$result_5_list['loan_code'].'</td>';
		...
		<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;background-color:orange;">'.$result_5_list['amount'].'</td></tr>';}
$ctr2=0;
		...
		<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;"></td>
		<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;"></td>
		<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;"></td></tr>';}

$html.='	<tr><td style = "text-align:center;"></td><td colspan="6" style = "text-align:right;border-right:1px solid black;border-left:1px solid black;border-bottom: 1px solid black;">TOTAL</td>
			<td style = "text-align:center;border-right:1px solid black;border-bottom: 1px solid black;">'.number_format($totalamount_instalment).'</td></tr>
		</table></div>';
