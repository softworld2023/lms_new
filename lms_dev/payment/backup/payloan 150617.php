...
<table width="1280" id="list_table">
	...
	<tr <?php echo $style; ?>>
		...
		<input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } ?>" /><a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save schedule date"><img src="../img/document_save.png" /></a><?php } ?></td>
   	  	<td><?php if($get_l['payment_date'] == '0000-00-00') {  ?> <?php if($style == '') {?>
			<input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y', strtotime($get_l['next_paymentdate'])); ?>" /><input type="hidden" name="tdm" id="tdm" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" />
            <input type="text" onblur="changeM(this.value)" name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" />
