...
<table width="1280" id="list_table">
    ...
    <tr>
      ...
      <td style="padding:0px">Interest Rate 
        ...
        <?php } ?></td>
      <td><input type="text" name="intrate" id="intrate" style="width:40px" value="<?php echo $intrate['int_percent']; ?>%" />
	  <input type="hidden" name="intratehide" id="intratehide" style="width:40px" value="<?php echo $intrate['int_percent']; ?>" /></td>
    ...
    <tr>
      <td style="padding:0px">Notes <?php if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff')) { ?>
...
?>
	<tr <?php echo $style; ?>>
      	...
      	<td><?php if($_SESSION['login_username'] != 'softworld' && $_SESSION['login_username'] != 'fong' && $_SESSION['login_username'] != 'staff') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } else { ?><input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } ?>" /><a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save schedule date"><img src="../img/document_save.png" /></a><?php } ?></td>
            ...
            <input type="text" name="payment_date" <?php if($get_q['loan_package'] != 'SKIM G')  echo 'onblur="changeM(this.value)"'; ?> id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" />
            <?php } else { if($get_q['loan_package'] == 'SKIM G') {?><input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y', strtotime($get_l['next_paymentdate'])); ?>" /><input type="hidden" name="tdm" id="tdm" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" /><input type="text" name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" /><?php } else{?><input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y'); ?>" /><input type="hidden" name="tdm" id="tdm" value="<?php echo date('m'); ?>" />
            <?php //if($latectr == $latecount) { ?>
            <input type="text" name="payment_date" id="payment_date" <?php if($get_q['loan_package'] != 'SKIM G')  echo 'onblur="changeM(this.value)"'; ?> style="width:80px" value="<?php echo date('d-m-Y'); ?>" />
            <?php //} else { ?>
            <?php //} ?>
            <?php }} ?>
		...
		?>
   	  	<td><input type="hidden" name="monthly" id="monthly" value="<?php echo number_format($ccm, '0', '.', ''); ?>" />
			<?php $totalp = number_format(($get_l['pokok'] + $get_l['interest']), '2'); ?>
			<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?>
			<input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" /><input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccm, '2'); ?>" />
			<?php } else { ?>
			<?php //if($latectr == $latecount) { ?>
				<input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" /><input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccm, '2'); ?>" />
      ...
      <td><input type="hidden" name="new_receipt" id="new_receipt" style="width:120px" value="<?php echo $get_q['loan_code']; ?>"/>
				...
				<?php } else if($get_q['loan_package'] == 'SKIM E') { ?>
				<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckE(this.value)" />
				<?php } else { ?>
				...
				<?php } else if($get_q['loan_package'] == 'SKIM E') { ?>
				<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckE(this.value)" />
				<?php } else if($get_q['loan_package'] == 'SKIM G'){ ?>
				<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckG(this.value)" />
				<?php } else { ?>				
				<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheck1(this.value)" />
		 		<?php } ?>
		  <?php } ?>
...
function payConfirm(period, month)
{
	...
	else
	{
		...
		$ccm = document.getElementById('bal_amount6').value;	
