...
<table width="1280" id="list_table">
    ...
    <tr <?php echo $style; ?>>
		...
		<?php
			...
			$mbal = $get_l['monthly'] - $prevpay['payment'];
		...
		<input type="hidden" name="monthly" id="monthly" value="<?php echo number_format($mbal, '0', '.', ''); ?>" />
            <?php if($style == '') {?>
            <input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" />
   	  	  <input type="text" name="payment" id="payment" class="currency" style="width:130px" placeholder="RM <?php echo $get_l['monthly']; ?>" onkeyup="monthlyCheck(this.value)" autocomplete="off" />
            <?php } else { ?>
            <?php // if($latectr == $latecount) { ?>
            <input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" />
   	  	  <input type="text" name="payment" id="payment" class="currency" style="width:130px" placeholder="RM <?php echo $get_l['monthly']; ?>" autocomplete="off" onkeyup="monthlyCheck1(this.value)" />
            <?php // } else { ?>
            <?php // } ?>
            <?php } ?>
        ...
        <td>
            ...
            <?php }else { ?>
			<?php if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff')) { ?>
			<?php $checkrow_q = mysql_query("SELECT * FROM loan_payment_details WHERE id > '".$get_l['id']."' AND customer_loanid = '".$id."'"); 
