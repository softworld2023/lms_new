...
$cust_id = $_GET['id'];
	...
	// // Get current year-month in format Y-m (e.g., "2025-05")
	// $current_month = date("Y-m");

	// // Step 1: Check if there's any record for this month
	// $check_sql = mysql_query("SELECT COUNT(*) as count FROM loan_payment_details WHERE customer_loanid = '$cust_id' AND DATE_FORMAT(payment_date, '%Y-%m') = '$current_month'");
	// $check_result = mysql_fetch_assoc($check_sql);

	// if ($check_result['count'] == 0) {
	// 	// Step 2: No record for this month — use the latest one
	...
	// } else {
	// 	// Step 3: Record exists — get the second latest
	// 	$sql2 = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '$cust_id' ORDER BY id DESC LIMIT 2");
	// 	$i = 0;
	// 	while ($row = mysql_fetch_assoc($sql2)) {
	// 		if ($i == 1) {
	// 			$loan_percent = $row['loan_percent'] - $row['payment'];
	// 		}
	// 		$i++;
	// 	}
	// }
