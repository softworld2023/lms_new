<?php
    ...
    if (isset($_POST)) {
        ...
        $sql = "SELECT
                ...
                FROM
                    $db.collection
                LEFT JOIN $db.customer_loanpackage ON customer_loanpackage.loan_code = collection.loan_code
                LEFT JOIN $db.customer_details ON customer_details.id = customer_loanpackage.customer_id  OR customer_details.customercode2 = customer_loanpackage.customer_id
                WHERE
        ...
        while ($row = mysql_fetch_assoc($query)) {
            ...
            $name = $row['name'];
            if(empty($name)){
                $sql = "SELECT * FROM $db.monthly_payment_record mpr LEFT JOIN $db.customer_details cust ON cust.id = mpr.customer_id WHERE mpr.loan_code = '$loan_code' ORDER BY mpr.id DESC LIMIT 1";
                $cust_detail = mysql_fetch_assoc(mysql_query($sql));
                $name = $cust_detail['name'];
                $customer_id = $cust_detail['customercode2'];
            }
            $salary = $row['salary'];
            $salary_type = $row['salary_type'];
            // $instalment = !empty($row['collection_amount']) ? $row['collection_amount'] : $row['instalment'];
            ...
            $action = "";
            if (!empty($filename)) {
                $action .= '<a href="/lms/collection_photo/' . $db . '/' . $loan_code . '/' . str_replace(array('-', ' ', ':'), '', $datetime) . '/' . $filename . '" target="_blank" title="View photo" style="text-decoration: none;">
            ...
            if ($status == 'PENDING' && $instalment_type != 'Monthly' && $instalment_type != 'Half Month') {
                $action .= '<button id="btn-approve" onclick="approveCollection(\'' . $collection_id . '\')" title="Approve" style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
            ...
            if ($status == 'PENDING' && $instalment_type == 'Half Month') {
                $action .= '<button id="btn-approve" onclick="approveCollection3(\'' . $collection_id . '\')" title="Approve" style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
                                <svg width="24px" height="24px" viewBox="0 0 512 512" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <style type="text/css">
                                        .st0{fill:#333333;}
                                        .st1{fill:none;stroke:#333333;stroke-width:32;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;}
            ...
            if($salary_type == 'Bad Debt'){
                ...
                $bd_row = mysql_fetch_assoc($bd_query);
                if ($bd_row) {
                    $bad_debt_id = $bd_row['id'];
                    var_dump($bad_debt_id); // for debugging
                } else {
                    die("No record found for loan_code = $loan_code");
                }
                $action .= '<button id="btn-delete" onclick="deleteBadDebt(\'' . $bad_debt_id . '\')" title="Delete Bad Debt" style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
            ...
            //         $action .= '<button id="btn-delete" onclick="deletex(\'' . $collection_id . '\')" title="Delete" style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
            //                     <svg xmlns="http://www.w3.org/2000/svg" fill="#323232" width="24px" height="24px" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            //                 </button>';
            //     } else {
            //         $action .= '<button id="btn-delete" onclick="deleteCollection(\'' . $collection_id . '\')" title="Delete" style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
            //                     <svg xmlns="http://www.w3.org/2000/svg" fill="#323232" width="24px" height="24px" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            //                 </button>';
            //     }
            // }
                ...
                if ($instalment_type == 'Half Month' && $status == 'APPROVED') {
                    ...
                    // =============================
                    //  Check latest Half Month record
                    // =============================
                    ...
                    $latest_row = mysql_fetch_assoc($latest_res);
                    $latest_halfmonth_id = $latest_row ? $latest_row['id'] : null;

                    // Only allow delete if this row is the latest Half Month
                    if ($collection_id == $latest_halfmonth_id) {
                        $action .= '<button id="btn-delete" onclick="deleteCollection(\'' . $collection_id . '\')" 
                                        title="Delete" 
                                        style="text-decoration: none; cursor: pointer; background-color: transparent; border: none;">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="#323232" width="24px" height="24px" viewBox="0 0 24 24">
            ...
            if($salary_type == 'Gaji' && $instalment_type != 'Monthly') {
                ...
                $html .='<td style="border-right:1px solid black;border-bottom: 1px solid black;color: black;"><b> Short
                 </b></td>
                        <td style="border-right:1px solid black;border-bottom: 1px solid black;color: black;"><b>RM ' . number_format($instalment, 2) . ' (' . $instalment_type . ') </b></td>
                        <td style="border-right:1px solid black;border-bottom: 1px solid black;color: black;"><b>RM ' . number_format($tepi1, 2) . '</b></td>
                        <td style="border-right:1px solid black;border-bottom: 1px solid black;color: black;"><b>RM ' . number_format($tepi2, 2) . '</b></td>
            ...
            $html .='<td style="border-right:1px solid black;border-bottom: 1px solid black;color: black; text-align: center; cursor: pointer;">' . $action . '</td>
                    </tr>';
        }
