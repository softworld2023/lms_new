...
/*if($_POST['action'] == 'payloanf')
{
	...
	$sql = mysql_query("SELECT * FROM loan_lejar_details WHERE id = '".$id."'");
	...
	$update_q = mysql_query("UPDATE loan_lejar_details SET payment = '".$amount."', payment_int = '".$int_amount."', payment_date = '".$date."' WHERE id = '".$id."'");
	...
	if($update_q)
	{
		...
		if($newmonth <= $period)
		{
			...
			if($newbalance > 0) 
			{
				...
				if($new_intbalance > 0)
				{
					$insert_new = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$newmonth."', int_percent = '".$get_q['int_percent']."', balance = '".$newbalance."', int_balance = '".$new_intbalance."', next_paymentdate = '".$next_paymentdate."', package_id = '".$get_q['package_id']."', receipt_no = '".$receipt_no."'");
			...
			else
			{
				...
				if($new_intbalance > 0)
				{
					$insert_new = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$newmonth."', int_percent = '".$get_q['int_percent']."', balance = '0', int_balance = '".$new_intbalance."', next_paymentdate = '".$next_paymentdate."', package_id = '".$get_q['package_id']."', reciept_no = '".$receipt_no."'");
				...
				{
					$insert_new = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$newmonth."', int_percent = '".$get_q['int_percent']."', balance = '0', int_balance = '0', package_id = '".$get_q['package_id']."', receipt_no = '".$receipt_no."'");
		...
		}else
		{
			...
			else
			{
				...
				if($new_intbalance > 0)
				{
					$insert_new = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$newmonth."', int_percent = '".$get_q['int_percent']."', balance = '0', int_balance = '".$new_intbalance."', next_paymentdate = '".$next_paymentdate."', package_id = '".$get_q['package_id']."', receipt_no = '".$receipt_no."'");
...
if($_POST['action'] == 'payloanf2') //flexi tapi resit guna code yang sama
{
	...
	$update_q = mysql_query("UPDATE loan_lejar_details SET payment = '".$amount."', payment_int = '".$int_amount."', payment_date = '".$date."', staff_id = '".$_SESSION['taplogin_id']."', staff_name = '".$_SESSION['login_name']."', created_date = now() WHERE id = '".$id."'");
	...
	if($new_loan == '' || $new_loan == 0)
	{
		...
		$updateloanbal = mysql_query("UPDATE customer_loanpackage SET 
										loan_total = '".$loan_balance."' WHERE 
										id = '".$get_q['customer_loanid']."'");
		...
		if($loan_balance > 0)
		{
			...
			if($amount == 0)
			{
				...
				$cashbook3_q = mysql_query("INSERT INTO cashbook SET 
												package_id = '".$get_q['package_id']."', 
												type = 'COMMISSION', 
												amount = '".$int_amount."', 
												transaction = 'KOMISYEN2', 
												customer_id = '".$get_lp['customer_id']."', 
				...
				$balance2_ql = mysql_query("INSERT INTO balance_rec SET 
												package_id = '".$get_q['package_id']."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												interest2 = '".$int_amount."', 
				...
				$nxmr_q = mysql_query("SELECT * FROM loan_lejar_details 
				...
				if(!$nxmr)
				{
					//insert new loan_lejar_details
					$nm = $get_q['month'] + 1;
					$insert_lp_q = mysql_query("INSERT INTO loan_lejar_details 
												SET customer_loanid = '".$get_q['customer_loanid']."', 
												month = '".$nm."', 
												int_percent = '".$get_q['int_percent']."', 
												balance = '".$loan_balance."',
												next_paymentdate = '".$next_paymentdate."', 
												package_id = '".$get_q['package_id']."', 
												receipt_no = '".$receipt_no."', 
												month_receipt = '".$month_receipt."', 
												branch_id = '".$_SESSION['login_branchid']."', 
				...
				else
				{
					...
					$insert_lp_q = mysql_query("UPDATE loan_lejar_details SET 
													balance = '".$loan_balance."' WHERE 
													id > '".$id."' AND 
													customer_loanid = '".$get_q['customer_loanid']."' AND 
													branch_id = '".$_SESSION['login_branchid']."'");
			...
			}else
			{
				...
				$cashbook_q = mysql_query("INSERT INTO cashbook SET package_id = '".$get_q['package_id']."', 
												type = 'RECEIVED', 
												amount = '".$amount."', 
												transaction = 'CCM', 
												code = '".$loancode."', 
												receipt_no = '".$get_q['receipt_no']."', 
												customer_id = '".$get_lp['customer_id']."', 
												table_id = '".$id."', 
												date = '".$date."', 
												staff_id = '".$_SESSION['taplogin_id']."', 
				...
				$balance1_qr = mysql_query("INSERT INTO balance_transaction SET 
												package_id = '".$get_q['package_id']."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												received = '".$amount."', 
												date = '".$date."', 
				...
				$balance2_qr = mysql_query("INSERT INTO balance_rec SET 
												package_id = '".$get_q['package_id']."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												received = '".$amount."', 
												month_receipt = '".$month_receipt."', 
				...
				if($int_amount != 0)
				{
					$balance2_qri = mysql_query("INSERT INTO balance_rec SET 
													package_id = '".$get_q['package_id']."', 
													customer_loanid = '".$get_q['customer_loanid']."', 
													interest2 = '".$int_amount."', 
													month_receipt = '".$month_receipt."', 
				...
				//check next month rec
				$nxmr_q = mysql_query("SELECT * FROM loan_lejar_details WHERE 
										id > '".$id."' AND 
										customer_loanid = '".$get_q['customer_loanid']."'");							
				$nxmr = mysql_fetch_assoc($nxmr_q);
				...
				if(!$nxmr)
				{
					//insert new loan_lejar_details
					$nm = $get_q['month'] + 1;
					$insert_lp_q = mysql_query("INSERT INTO loan_lejar_details SET 
													customer_loanid = '".$get_q['customer_loanid']."', 
													month = '".$nm."', 
													int_percent = '".$get_q['int_percent']."', 
													balance = '".$loan_balance."',
		...
		{
			//insert to cashbook rec the actual amount paid
			$cashbook_q = mysql_query("INSERT INTO cashbook SET 
										package_id = '".$get_q['package_id']."', 
										type = 'RECEIVED', 
										amount = '".$amount."', 
										transaction = 'CCM', 
			...
			{				
				...
				$cashbook3_q = mysql_query("INSERT INTO balance_rec SET 
												package_id = '".$get_q['package_id']."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												interest2 = '".$int_amount."',  
												month_receipt = '".$month_receipt."', 
			...
			//set loan = FINISHED
			$update_lp = mysql_query("UPDATE customer_loanpackage SET 
										loan_status = 'Finished' WHERE 
										id = '".$get_q['customer_loanid']."'");
	...
	{	
		...
		if($amount != 0)
		{
			...
			//check next month rec
			$nxmr_q = mysql_query("SELECT * FROM loan_lejar_details WHERE id > '".$id."' AND customer_loanid = '".$get_q['customer_loanid']."'");							
			$nxmr = mysql_fetch_assoc($nxmr_q);
			...
			{
				$insert_lp_q = mysql_query("UPDATE loan_lejar_details 
											SET balance = '".$new_loan."', 
											receipt_no = '".$receipt_no."', 
											branch_id = '".$_SESSION['login_branchid']."', 
											branch_name = '".$_SESSION['login_branch']."' 
											WHERE id > '".$id."' AND customer_loanid = '".$get_q['customer_loanid']."'");
		...
		{// just add loan no payment
			...
			{
				$insert_lp_q = mysql_query("UPDATE loan_lejar_details SET balance = '".$loan_balance."', receipt_no = '".$receipt_no."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."' WHERE id > '".$id."' AND customer_loanid = '".$get_q['customer_loanid']."'");
...
if($_POST['action'] == 'payloanf')//tukar resit
{
	...
	if($new_loan == '' || $new_loan == 0)
	{
		...
		if($loan_balance > 0)
		{
			...
			if($amount == 0)
			{
				...
				{
					//update receipt no
					$insert_lp_q = mysql_query("UPDATE loan_lejar_details SET balance = '".$loan_balance."', receipt_no = '".$receipt_no."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."' WHERE customer_loanid = '".$get_q['customer_loanid']."' AND id > '".$id."'");
		...
		}else
		{
			...
			//check next month rec
				...
				if(!$nxmr)
				{	
					$insert_lp_q = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$nm."', int_percent = '".$get_q['int_percent']."', balance = '0', package_id = '".$get_q['package_id']."', receipt_no = '".$receipt_no."', month_receipt = '".$month_receipt."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."'");
	...
	{	
		...
		if($amount != 0)
		{
			...
			{
				//update receipt no
				$insert_lp_q = mysql_query("UPDATE loan_lejar_details SET balance = '".$new_loan."', receipt_no = '".$receipt_no."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."' WHERE customer_loanid = '".$get_q['customer_loanid']."' AND id > '".$id."'");
...
if($_POST['action'] == 'delete_payment')
{
	...
	$delete_q2 = mysql_query("DELETE FROM cashbook_skimkutu 
								WHERE package_id = '".$skp['skim_id']."' 
								AND outamt = '".$skp['amount']."' 
								AND date = '".$skp['payment_date']."'");
	
	$delete_q = mysql_query("DELETE FROM skimkutu_payment 
								WHERE skim_id = '".$skp['skim_id']."' 
								AND amount = '".$skp['amount']."' 
								AND payment_date = '".$skp['payment_date']."' ");
...
if($_POST['action'] == 'delete_payment_kutuOffice')
{
	$id = $_POST['id'];
	$inamt = $_POST['inamt'];
	$outamt = $_POST['outamt'];	
	...
	$kop_q = mysql_query("SELECT * FROM kutuoffice_payment WHERE id = '".$id."'");
	$kop = mysql_fetch_assoc($kop_q);
	
	$delete_q = mysql_query("DELETE FROM cashbook_kutuoffice WHERE package_id = '".$kop['skim_id']."' AND inamt = '".$kop['inamt']."'AND outamt = '".$kop['outamt']."' AND date = '".$kop['payment_date']."'");
	
	$delete_q2 = mysql_query("DELETE FROM kutuoffice_payment WHERE id = '".$id."' AND inamt = '".$inamt."' AND outamt = '".$outamt."' AND payment_date = '".$payment_date."' ");
	
	$after_delete_q = mysql_query("SELECT * FROM kutuoffice_payment WHERE skim_id = '".$kop['skim_id']."' ");
	$rows_after_delete = mysql_num_rows($after_delete_q);
	
	if($rows_after_delete == 0)
	{
		mysql_query("DELETE FROM kutu_office WHERE id = '".$kop['skim_id']."'");		
	}
...
if($_POST['action'] == 'payloan')
{
	...
	$select = mysql_query("SELECT * FROM loan_lejar_details WHERE receipt_no = '".$prev_receipt."' ORDER BY id ASC");
	...
	//monthly 
	$m_q = mysql_query("SELECT * FROM loan_lejar_details WHERE customer_loanid = '".$get_q['customer_loanid']."' AND month = '1'");
	...
	if($ar == 0)
	{
		$update_q = mysql_query("UPDATE loan_lejar_details SET 
									payment = '".$amount."', 
									payment_date = '".$date."', 
									no_resit = '".$no_resit."', 
									staff_id = '".$_SESSION['taplogin_id']."', 
		...
		if($update_q)
		{
			...
			//check next record 
			$nextp_q = mysql_query("SELECT * FROM loan_lejar_details WHERE 
									customer_loanid = '".$get_q['customer_loanid']."' 
									AND id > '".$id."' ");
			...
			$newmonth = $month + 1;if(!$nextp)
			{
				...
				//{
					...
					if($newbalance > 0)
					{
						$insert_new = mysql_query("INSERT INTO loan_lejar_details SET 
													customer_loanid = '".$get_q['customer_loanid']."', 
													month = '".$newmonth."', 
													int_percent = '".$get_q['int_percent']."', 
													balance = '".$newbalance."', 
													monthly = '".$get_mq['monthly']."', 
													pokok = '".$pokok."', 
													interest = '".$get_q['interest']."', 
													next_paymentdate = '".$next_paymentdate."', 
													package_id = '".$get_q['package_id']."', 
													receipt_no = '".$receipt_no."', 
													branch_id = '".$_SESSION['login_branchid']."', 
					...
					else
					{
						...
						if($insert_new)
						{
							...
							// 							WHERE id = '".$get_q['customer_loanid']."'");
				...
				/* }
					...
					$insert_new = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$get_q['customer_loanid']."', month = '".$newmonth."', balance = '0', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."'");
			...
			}else
			{
				...
				// update balance
				$insert_new = mysql_query("UPDATE loan_lejar_details SET 
											balance = '".$newbalance."', 
											monthly = '".$get_mq['monthly']."', 
											pokok = '".$pokok."', 
											interest = '".$get_q['interest']."', 
...
if($_POST['action'] == 'update_period')
{
	...
	$update_q = mysql_query("UPDATE customer_loanpackage SET 
							loan_period = '".$newP."' WHERE 
							id = '".$id."'");
...
if(isset($_POST['adjust_loan']))
{
	...
	$payment_q = mysql_query("SELECT * FROM loan_lejar_details WHERE id = '".$payment_id."'");
	...
	if($ar != 0)
	{
		if($get_l['loan_period'] != $month)
		{
			...
			if($update_l)
			{
				...
				$update_q = mysql_query("UPDATE loan_lejar_details SET interest = '".$new_intpermonth."', balance = '".$new_totbal."', monthly = '".$new_monthly."' WHERE id = '".$payment_id."'");
	...
	if($ar != 0)
	{
		if($get_l['loan_period'] != $month)
		{
			...
			if($update_l)
			{
				...
				$update_q = mysql_query("UPDATE loan_lejar_details SET int_balance = '".$new_int."' WHERE id = '".$payment_id."'");
...
{  // --- get from payloan_CEK ---
	...
	$sql = mysql_query("SELECT * FROM loan_lejar_details WHERE 
							id = '".$id."'");
	$get_q = mysql_fetch_assoc($sql);
	
	$loan_q = mysql_query("SELECT * FROM customer_loanpackage WHERE 
							id = '".$get_q['customer_loanid']."'");
	$get_l = mysql_fetch_assoc($loan_q);
	
	$update_q = mysql_query("UPDATE loan_lejar_details 
	...
	if($update_q)
	{//after perform the above sql
		//minus the balance after customer paying the firm!
		$balance = $get_q['balance'] - $amount;
		//update balance 
		$update_bal_q = mysql_query("UPDATE loan_lejar_details 
										SET balance = '".$balance."', 
										branch_id = '".$_SESSION['login_branchid']."', 
										branch_name = '".$_SESSION['login_branch']."' 
										WHERE customer_loanid = '".$get_q['customer_loanid']."' 
		...
		if($update_bal_q)
		{//after executiing the above sql
			//if balance = 0
			if($balance == 0)
			{//if there is no more balance
				
				//month is just an increment number
				$nm = $get_q['month'] + 1;
				$insert_bal_q = mysql_query("INSERT INTO loan_lejar_details SET 
												balance = '".$balance."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												month = '".$nm."'");
				
				$updatelp = mysql_query("UPDATE customer_loanpackage SET 
											loan_status = 'Finished' WHERE 
											id = '".$get_q['customer_loanid']."'");
			}
...
if($_POST['action'] == 'payloan_CEKccm')
{
	...
	$sql2_q = mysql_query("SELECT * FROM loan_lejar_details WHERE customer_loanid = '".$id."' AND payment_date = '0000-00-00' ORDER BY month ASC");
	$get_q2 = mysql_fetch_assoc($sql2_q);
	
	if($ptype == 'CCM')
	{
		//insert payment
		$insert_q = mysql_query("INSERT INTO loan_lejar_details SET customer_loanid = '".$id."', int_percent = '".$get_q2['int_percent']."', balance = '".$get_q2['balance']."', payment_date = '".$date."', payment = '".$amount."', package_id = '".$get_q2['package_id']."', receipt_no = '".$get_q2['receipt_no']."'");
	}else
	{//$ptype = INTEREST	
