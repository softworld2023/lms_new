<?php
    ...
    // $closing_balance = $opening_balance + $total_instalment_collected + $total_settle + $capital_in + $total_bd_collected + $total_monthly - $total_instalment_payout - $total_expenses - $total_expenses_2 - $total_interest_paid_out - $account_capital;
    ...
    // echo $account_capital . '<br>';
    // echo $closing_balance . '<br>';

    $sql = "SELECT
                initial_closing_balance_instalment
            FROM $db.account_book_instalment
            WHERE year = '$selected_year' AND month = '$selected_month'";
    $q = mysql_query($sql);
    $res = mysql_fetch_assoc($q);
    $initial_closing_balance_instalment = isset($res['initial_closing_balance_instalment']) ? $res['initial_closing_balance_instalment'] : 0;
...
#tbl-account-book-instalment {
    border-collapse:collapse;
    border:none;	
}

#tbl-account-book-instalment tr th {
    height:36px;
    background:#666;
    text-align:left;
    padding-left:10px;
...
#tbl-account-book-instalment tr td {
    height:35px;
    padding-left:10px;
    padding-right:10px;
}

#btn-search-account-book-monthly {
    cursor: pointer;
}

#initial-closing-balance-container {
    display: flex;
...
@media print {
    ...
    #tbl-account-book-instalment tr {
        page-break-inside: avoid;
    }

    /* Make table header repeat on each page */
    #tbl-account-book-instalment thead {
        display: table-header-group;
    }

    #tbl-account-book-instalment tfoot {
    ...
    #search,
    #search1,
    #btn-search-account-book-instalment,
    input,
    select,
    label,
    .customBtn {
...
<table width="1280">
        	...
        	<table>
				<tr>
                    ...
                    <td style="padding-right:8px">
                    	<input type="button" id="search" name="search" value="" onclick="searchAccountBookInstalment();"/>
					</td>
                </tr>
	...
	<tr>
        ...
        <div class="subnav">
            ...
            <a href="account_book_instalment.php" id="active-menu">Account Book (Instalment)</a>
		</div>
        </td>
    </tr>
...
<table width="1280" id="list_table">
             ...
             <table width="100%" border='0'>
                 <tr>              
                     <td style="font-size: 16px;"><br>YEAR
                         ...
                         <input class="btn btn-blue" type="button" id="btn-search-account-book-instalment" name="search_account_book_instalment" value="Search" style="width: 100px; height: 30px; font-size:16px;" onclick="searchYearMonth();">
                     </td>
 ...
 <div id="initial-closing-balance-container">
    <div style="margin-right: 5px;"><b>Balance:</b></div>
    <div>
        <input type="number" id="initial_closing_balance" name="initial_closing_balance" style="height: 30px; width: 100px;" value="<?php echo $initial_closing_balance_instalment; ?>"/>
    </div>
    <div style="margin-left: 5px;">
        <button type="button" id="btn-save" style="cursor: pointer;">Save</button>
    </div>
</div>
<div id="print-only-header">
    <div style="font-size: 20px; font-weight: bold;">ACCOUNT BOOK (INSTALMENT)</div>
    <div id="print-only-date" style="margin-top: 5px; font-size: 16px;"></div>
</div>
 <table width="1280" id="tbl-account-book-instalment">
    <thead>
        ...
        <tr>
            ...
            <th width="20%" style="border-right:1px solid black;border-bottom: 1px solid black;border-top:1px solid black;">Balance</th>
        </tr>
    </thead>
    <tbody>
    </tbody>
...
<script>
    ...
    $('#btn-save').on('click', function() {
        $.ajax({
            url: 'update_account_book_instalment_ajax.php',
            type: 'POST',
            data: {
                year: $('#selected_year').val(),
                month: $('#selected_month').val(),
                initial_closing_balance: $('#initial_closing_balance').val()
            },
    ...
    function searchAccountBookInstalment() {
        $.ajax({
            url: 'get_account_book_instalment_listing_ajax.php',
            type: 'POST',
            data: {
                year: $('#selected_year').val(),
                month: $('#selected_month').val(),
            ...
            success: function(response) {
                // console.log(response);
                $('#tbl-account-book-instalment tbody').html(response);
            }
        });
    }
    ...
    function searchYearMonth() {
        ...
        window.location.href = 'account_book_instalment.php?year=' + year + '&month=' + month;
    }
</script>