...
$cust_id = $_GET['id'];
	...
	$sql = "SELECT balance FROM loan_lejar_details WHERE receipt_no = '".$loan_code."' ORDER BY id ASC LIMIT 1";
	$query = mysql_query($sql);
	$result = mysql_fetch_assoc($query);

	$balance = $result['balance'];
	$jumlah_faedah = $balance - $loan_pokok;
...
<center>
<form method="post" action="action_settlement.php" onSubmit="return checkForm()"><input type="hidden" name="custid" id="custid" value="<?php echo $cust_id; ?>">
        ...
        <tr>
          <td style="padding-left:10px; padding-right:10px">	
            ...
            ?>						
		  	...
		  	<tr>
					<td>Pinjaman Pokok (RM)</td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="loan_pokok" id="loan_pokok" class="currency" value="<?php echo $loan_pokok;?>" readonly/></td>
				</tr>
				<tr>
					<td>Jumlah Faedah (RM)</td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="jumlah_faedah" id="jumlah_faedah" class="currency" value="<?php echo $jumlah_faedah;?>" readonly /></td>
				</tr>
		  	<tr>
					<td>Jumlah Besar (RM)</td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="loan_total" id="loan_total" class="currency" value="<?php echo $balance;?>" readonly/></td>
				</tr>
				<tr>
					<td>Tempoh Bayaran (Bulan) </td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="loan_period" id="loan_period" class="currency" value="<?php echo $loan_period;?>" readonly/></td>
				</tr>
				<tr>
					<td>Baki Pinjaman (RM)</td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="baki_pinjaman" id="baki_pinjaman" class="currency" value="<?php echo $baki_pinjaman;?>" readonly/></td>
				</tr>
				<tr>
					<td><b>Settlement Period</b></td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="settlement_period" id="settlement_period"  onKeyUp="cal()" /></td>
				</tr>
				<tr>
					<td>Settlement Amount</td>
					<td>&nbsp;</td>
					<td colspan="4"><input type="text" name="settlement_amount" id="settlement_amount" class="currency" readonly="" /></td>
				</tr>
			</table>
		  </td>
        </tr>
...
<script>
$('#settle_date').click( function(e) {$(this).off('click').AnyTime_picker({ format: "%d-%m-%Y", labelTitle: "Select Date"}).focus(); } ).
keydown(
	function(e)
	{
