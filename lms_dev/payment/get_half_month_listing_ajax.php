...
if ( isset( $_POST ) ) {
    ...
    $sql = "
        SELECT 
            ...
            hpd.payment_date
        FROM 
            $db.half_payment_details hpd
        JOIN 
            $db.customer_details cd 
            ON cd.id = hpd.customer_id
        WHERE 
            hpd.month_receipt = '$selected_year-$selected_month'
            AND hpd.status != 'DELETED'
        ORDER BY hpd.payment_date ASC
    ";

    if ( $loan_code != '' ) {
        $sql .= " AND hpd.loan_code = '$loan_code'";
    }
...
// var_dump($sql);
    ...
    while ( $row = mysql_fetch_assoc( $query ) ) {
        ...
        if ( !isset( $grouped_data[ $key ] ) ) {
            $grouped_data[ $key ] = array(
                ...
                'first_monthly_date' => $row[ 'payment_date' ],
                'total_payout' => 0,
                'ori_instalment' => $row[ 'ori_instalment' ],
                'records' => array()
            );
        }

        // Update earliest date
        if ( strtotime( $row[ 'payment_date' ] ) < strtotime( $grouped_data[ $key ][ 'first_monthly_date' ] ) ) {
            $grouped_data[ $key ][ 'first_monthly_date' ] = $row[ 'payment_date' ];
        }

        // Add payout amount
        $grouped_data[ $key ][ 'total_payout' ] += $row[ 'payment' ];
    ...
    foreach ( $grouped_data as $group ) {
        ...
        $html .= '<tr>
            ...
            <td style="border:1px solid black; text-align:right; color: black; font-weight: bold;">RM ' . number_format( $group[ 'ori_instalment' ], 2 ) . '</td>
        ...
        if ( count( $group[ 'records' ] ) > 1 ) {
            ...
            foreach ( $group[ 'records' ] as $record ) {
                $html .= '<tr>
                    <td style="text-align:center;">' . date( 'd/m/Y', strtotime( $record[ 'payment_date' ] ) ) . '</td>
                    <td style="text-align:center;">' . htmlspecialchars( $record[ 'loan_code' ] ) . '</td>
                    <td style="text-align:right;">RM ' . number_format( $record[ 'payment' ], 2 ) . '</td>
                </tr>';
            }
