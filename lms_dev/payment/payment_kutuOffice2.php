...
$sql = mysql_query("SELECT * FROM  kutu_office WHERE id = '".$id."'");
$get_q = mysql_fetch_assoc($sql);

$payment = mysql_query("SELECT * FROM kutuoffice_payment WHERE skim_id = '".$id."' ORDER BY month ASC");
...
<tr>
	...
	<td>Payment Kutu Office <strong><?php echo $get_q['title']; ?> 
	...
	<td colspan="3">
		...
		<tr>
			...
			<?php if($paymentdone != $get_q['period']) { ?>
				<a href="kutuOfficePayment.php?id=<?php echo $id; ?>" title="Make Payment" rel="shadowbox; width=800px; height=380px" >
					<table>
					<tr>
...
<?php 
	...
	while($stock_amt = mysql_fetch_assoc($stock_amt_q)){
...
?> 
<tr>
    <td><?php echo $ctr; ?></td><!-- No. -->
    <td><?php echo date('d/m/Y', strtotime($get_p['payment_date'])); ?></td><!-- Date -->
    <td><?php if($get_p['inamt'] != 0) { echo "RM ".number_format($get_p['inamt'],'2'); }?></td><!-- In -->
	<td><?php if($get_p['outamt'] != 0) { echo "RM ".number_format($get_p['outamt'],'2'); }?></td>	<!-- Out -->
    <td><?php /*echo $stock_left;*/echo "RM ".number_format($get_p['amount'],'2'); ?></td><!-- Stock -->
	<td>
		...
		<tr>
			...
			<td style="padding:0px">
			  <input type="text" size="8" name="editdate_<?php echo $ctr; ?>" id="editdate_<?php echo $ctr; ?>" value="<?php echo $get_p['payment_date'];  ?>">
			</td>
			<td>Edit In</td>
			<td style="padding:0px">
			  <input type="text" size="8" name="editin_<?php echo $ctr; ?>" id="editin_<?php echo $ctr; ?>" value="<?php echo $get_p['inamt']; ?>">
			</td>			
			<td>Edit Out</td>
			<td style="padding:0px">
			  <input type="text" size="8" name="editout_<?php echo $ctr; ?>" id="editout_<?php echo $ctr; ?>" value="<?php echo $get_p['outamt']; ?>">
			</td>			
			<td>
...
<?php 
	...
	{ 
		$checkrow_q = mysql_query("SELECT * FROM kutuOffice_payment WHERE id > '".$get_p['id']."' AND skim_id = '".$id."'");   
		$checkrow = mysql_num_rows($checkrow_q);
		if($checkrow == 0)
		{
?>
		<center>
			<a href="javascript:deleteConfirmation('<?php echo $get_p['payment_date']; ?>', '<?php echo $get_p['id']; ?>', '<?php echo $get_p['inamt']; ?>','<?php echo $get_p['outamt']; ?>')">
...
<tr>
	<td colspan="8" align="right">
		<input type="button" name="back" id="back" onClick="window.location.href='kutuOffice.php'" value=""></td>
</tr>
<tr>
	<td colspan="8">&nbsp;</td>
</tr>
...
function deleteConfirmation(payment_date, id, inamt, outamt){
	$id = id;
	...
	$.confirm({
		...
		'message'	: 'Are you sure want to delete payment at ' + payment_date + ' ?',
		'buttons'	: {
			'Yes'	: {
			'class'	: 'blue',
			'action': function(){
				$.ajax({
						type: 'POST',
						data: {
							action: 'delete_payment_kutuOffice',
							id: $id,
							payment_date: $payment_date,
							inamt: $inamt,
							outamt: $outamt,
