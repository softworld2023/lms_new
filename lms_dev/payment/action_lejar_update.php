...
if($_POST['action'] == 'update_lejar_date')
{
	$id = $_POST['id'];
	if($_POST['loan_lejar_date']!='')
	{


	$loan_lejar_date = date('Y-m-d', strtotime($_POST['loan_lejar_date']));
}else
{
	$loan_lejar_date = '';
}
	...
	//update pokok
	$update_q = mysql_query("UPDATE customer_loanpackage SET loan_lejar_date = '".$loan_lejar_date."' WHERE id = '".$id."'");
	

	if($update_q)
	{
		$msg .= 'Date has been successfully updated.<br>';
...
if($_POST['action'] == 'insert_resit')
{
	...
	$update_q = mysql_query("UPDATE loan_lejar_details SET no_resit = '".$resit."' WHERE id = '".$id."'");
...
if($_POST['action'] == 'delete_payfixed')//delete for payloan_a.php
{
	...
	$pr_q = mysql_query("SELECT * FROM loan_lejar_details WHERE id = '".$id."'");
	$pr = mysql_fetch_assoc($pr_q);
	$loan_code = $pr['receipt_no'];

		$sql = mysql_query("UPDATE customer_loanpackage SET settlement = '',bad_debt = '' WHERE loan_code = '".$loan_code."'");
	
				//update payment_rec
				$update_q = mysql_query("UPDATE loan_lejar_details SET bad_debt = '',no_resit = '', payment = '', payment_date = '', loan_settle = '' WHERE id = '".$id."'");
			
				
				//delete from loan_payment_details	(next payment)
				$no_resit = mysql_query("DELETE FROM lejar_no_resit WHERE no_resit ='".$pr['no_resit']."'");
	$delete_payrec = mysql_query("DELETE FROM loan_lejar_details WHERE id > '".$id."' AND customer_loanid = '".$loanid."'");
