...
$loan_q = mysql_query("SELECT * FROM balance_transaction WHERE package_id = '".$package_id."' AND branch_id = '".$_SESSION['login_branchid']."' AND date LIKE '%".$month."%' ORDER BY date ASC");

$prevbal_q = mysql_query("SELECT SUM(loan) AS amtloan, SUM(received) AS amtrec FROM balance_transaction WHERE package_id = '".$package_id."' AND branch_id = '".$_SESSION['login_branchid']."' AND date < '".$newmth."'");
...
<table width="1280" id="list_table">
    ...
    while($get_l = mysql_fetch_assoc($loan_q)){
		...
		if($d != $prev_d)
		{
			$sum_q = mysql_query("SELECT SUM(loan) AS sumLoan, SUM(received) AS sumRec FROM balance_transaction WHERE date LIKE '%".$d."%' AND branch_id = '".$_SESSION['login_branchid']."' AND package_id = '".$package_id."' ORDER BY date ASC");
