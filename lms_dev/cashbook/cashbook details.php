...
$loan_q = mysql_query("SELECT * FROM cashbook WHERE branch_id = '".$_SESSION['login_branchid']."' AND package_id = '".$package_id."' AND date LIKE '%".$month."%' ORDER BY date ASC, id ASC");
...
<table width="1280" id="list_table">
    ...
    <tr style="border-bottom:thin solid #EEEEEE">
      <td><?php echo date('d/m/Y', strtotime($get_l['date'])); ?></td>
		...
		?>		</td>
        <td><?php if($out != '') { echo number_format($out, '2'); } if($get_p['scheme'] == 'SKIM CEK') { if($out1 != '') { echo number_format($out1, '2'); }
		} ?></td>
