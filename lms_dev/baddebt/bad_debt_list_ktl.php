...
// $bad_debt_q = mysql_query("SELECT * FROM ".$bd_name.".late_interest_record AS table1 LEFT JOIN ".$bd_name.".customer_details AS table2 ON table1.customer_id = table2.id LEFT JOIN ".$bd_name.".customer_employment AS table3 ON table1.customer_id = table3.customer_id LEFT JOIN ".$bd_name.".customer_address AS table4 ON table1.customer_id = table4.customer_id");
...
$branch_name = $_POST['branch_name'];
	$cond = '';
		...
		{
			$customer_sql = mysql_query("SELECT * FROM ".$bd_name.".customer_details AS table2 WHERE name = '".$_POST['customer_name']."'");
			$cust = mysql_fetch_assoc($customer_sql);
			$cond .= "where table2.id = '".$cust['id']."'";	
		}
		...
		if($_POST['customer_ic'] != '')
		{
			...
			$code_sql = mysql_query("SELECT * FROM ".$bd_name.".customer_details 
									WHERE nric = '".$_POST['customer_ic']."'");
			$code = mysql_fetch_assoc($code_sql);
			$cond .= "where table2.id = '".$code['id']."'";	

		}
		
		$statement =  "".$bd_name.".late_interest_record AS table1 LEFT JOIN ".$bd_name.".customer_details AS table2 ON table1.customer_id = table2.id LEFT JOIN ".$bd_name.".customer_employment AS table3 ON table1.customer_id = table3.customer_id LEFT JOIN ".$bd_name.".customer_address AS table4 ON table1.customer_id = table4.customer_id $cond";
						
}
else    // not click search and also no history result
{
	$statement = "".$bd_name.".late_interest_record AS table1 LEFT JOIN ".$bd_name.".customer_details AS table2 ON table1.customer_id = table2.id LEFT JOIN ".$bd_name.".customer_employment AS table3 ON table1.customer_id = table3.customer_id LEFT JOIN ".$bd_name.".customer_address AS table4 ON table1.customer_id = table4.customer_id ";
}
...
<td align="right">
       	<form action="bad_debt_list_ktl.php" method="post">
        	<table>
				<tr>
					<td align="right" style="padding-right:10px">Customer Name</td>
					<td>
					<?php
$sql="SELECT * FROM ktl.customer_details"; 
...
<?php } }else{
?> <tr id="tr_list" >
    	<td colspan="6" style="border-right:1px solid black;border-bottom: 1px solid black;border-left:1px solid black;color: black;text-align: center;">- No Records -</td>
    </tr>
<?php }?>
    <tr>
