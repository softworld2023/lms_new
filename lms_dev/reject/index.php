...
if(isset($_POST['search']))   // click search
{
	...
	// 	$code_sql = mysql_query("SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA') AND status = 'ACTIVE'");
	...
	if($_POST['customer_name'] != '')
	{
		...
		// $customer_sql = mysql_query("SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA') AND customer_name = '".$_POST['customer_name']."' AND status = 'ACTIVE'");
	...
	if($_POST['customer_ic'] != '')
	{
		...
		// $code_sql = mysql_query("SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA') AND customer_ic = '".$_POST['customer_ic']."' AND status = 'ACTIVE'");
		// $code = mysql_fetch_assoc($code_sql);
		// $cond .= "where customer_ic = '".$code['customer_ic']."'";	
		$cond .= " AND customer_ic = '$customer_ic'";
	}
	
	// $statement = "SELECT * FROM loansystem_dev.reject_loan ".$cond." ORDER BY reject_date ASC";
	...
	if ($login_branch == "ANSENG" || $login_branch == "MAJUSAMA" || $login_branch == "YUWANG" || $login_branch == "DK") {
		...
		$statement = "SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA' OR branch_name = 'YUWANG' OR branch_name = 'DK')" . $cond . " AND status = 'ACTIVE' ORDER BY reject_date ASC";
	} elseif ($login_branch == "KTL" || $login_branch == "TSY" || $login_branch == "TSY2") {
		$statement = "SELECT * FROM loansystem_dev.reject_loan WHERE 1=1 " . $cond . " AND status = 'ACTIVE' ORDER BY reject_date ASC";
	}		
}
else    // not click search and also no history result
{
		// $statement = "SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA') AND status = 'ACTIVE' ORDER BY reject_date ASC";

		// Check the value of $login_branch
		if ($login_branch == "ANSENG" || $login_branch == "MAJUSAMA" || $login_branch == "YUWANG" || $login_branch == "DK") {

			//If $login_branch is "ANSENG" or "MAJUSAMA" or "YUWANG", set $statement for ANSENG, MAJUSAMA, YUWANG, DK branches
			$statement = "SELECT * FROM loansystem_dev.reject_loan WHERE (branch_name = 'ANSENG' OR branch_name = 'MAJUSAMA' OR branch_name = 'YUWANG' OR branch_name = 'DK') AND status = 'ACTIVE' ORDER BY reject_date ASC";
		} elseif ($login_branch == "KTL" || $login_branch == "TSY" || $login_branch == "TSY2") {
			$statement = "SELECT * FROM loansystem_dev.reject_loan WHERE status = 'ACTIVE' ORDER BY reject_date ASC";
		}
}
       	...
       	<form action="index.php" method="post">
        	<table>
				<!-- <tr> <td align="right" style="padding-right:10px">Branch</td>
					...
					<td>
						<?php
							$sql="SELECT * FROM loansystem_dev.reject_loan WHERE branch_name = '$login_branch' AND status = 'ACTIVE' "; 
...
<table width="1500" id="list_table" >
    ...
    <tr id="tr_list">
        ...
        <td>
        	...
        	?>
			<a href="view_customer_loan.php?id=<?php echo $branch['customer_id']; ?>"><img src="../../img/customers/search.png" title="View" style=""></a>
        	<?php
        		}
        	?>
