...
<center>
<form method="post" action="action.php" onSubmit="return checkForm()"><input type="hidden" name="lperiod" id="lperiod" value="<?php echo $get_l['loan_period']; ?>">
<table width="100%" id="list_table" border="0">
    ...
    <tr>
    	<td width="40%" style="padding-right:10px;" align="right">Date</td>
    ...
    <tr>
      <td style="padding-right:10px;" align="right">Interest (Upfront 1 month) </td>
      <td align="left"><input type="text" name="intup" id="intup" style="width:40px" onKeyUp="calpay(this.value)"> %&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; RM <input type="text" name="intamt" id="intamt" style="border:none" readonly="readonly"></td>
    </tr>
    <tr>
      <td style="padding-right:10px;" align="right">Subtotal Loan Amount </td>
      <td align="left"><strong>RM</strong> 
        <input style="border:none; font-weight:bold" type="text" name="scash_pay" id="scash_pay" class="currency" onKeyUp="calculateTotal()" readonly="readonly" tabindex="1" />&nbsp;</td>
    </tr>
    <tr>
	...
	<tr>
        ...
        <td align="left"><strong>RM </strong>
          <input style="border:none; font-weight:bold" type="text" name="cash_pay" id="cash_pay" class="currency" onKeyUp="calculateTotal()" readonly="readonly" tabindex="1" /></td>
    </tr>
	...
	<tr>
	  ...
	  <td><span style="padding-right:10px;">
	    <input type="text" name="receipt_no" id="receipt_no" tabindex="3"  onBlur="checkReceipt(this.value)" value="<?php echo $get_l['loan_code']; ?>">
	  </span></td>
	  </tr>
	  
	  <tr>
	  	<td colspan="2">
			<div id="message2" style="width:100%; text-align:left"></div>		</td>
	  </tr>
	...
	<?php for($i = 1; $i <= $get_l['loan_period']; $i++)
	{ ?>
    <tr>
      <td style="padding-right:10px;" align="right">Payment Date <?php echo $i; ?></td>
      <td align="left"><input type="text" name="payment_date_<?php echo $i; ?>" id="payment_date_<?php echo $i; ?>" tabindex="3" /></td>
    </tr>
	<?php } ?>

   <tr height="20"></tr>
     <tr>
    	<td colspan="2" style="padding-right:10px" align="right"><input type="submit" name="pay_custCek" id="pay_custCek" value="" tabindex="4">&nbsp;&nbsp;&nbsp;<input type="reset" id="reset" name="reset" value=""></td>
    </tr>
</table>
...
$lp = document.getElementById('lperiod').value;
for($i = 1; $i <= $lp; $i++)
{
	$('#payment_date_'+$i).click( function(e) {$(this).off('click').AnyTime_picker({ format: "%d-%m-%Y", earliest:(new Date((new Date()).valueOf() + 1000*3600*24)), labelTitle: "Select Date"}).focus(); } ).
	keydown(
		function(e)
		{
...
function checkForm()
{
	if((document.getElementById('total_pay').value != ''))
	{
		if((document.getElementById('ltype').value != 'Flexi Loan'))
		{
			...
			if($x == '1')
			{
				var msg = "<div class='error'>You must enter the payment date(s)!</div>";
				$('#message2').empty();
				$('#message2').append(msg); 
				$('#message2').html();
				return false;
