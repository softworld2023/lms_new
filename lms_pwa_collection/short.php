...
<html lang="en">
    ...
    <body>
        ...
        <div id="collection" style="display: none;">
            <div class="app-bar p-3 pl-3" style="background-color: #6b88f5; height: fit-content;">
                <div class="d-flex flex-column flex-sm-row gap-2">
                    ...
                    <div>
                        ...
                        <a href="../payment/add_monthly.php" target="_blank" style="margin-left: 10px; text-decoration: none;">
                            <img src="../img/apply-loan/add-btn.jpg" title="Add Monthly">
                        </a>
                        <a href="../payment/add_instalment.php" target="_blank" style="margin-left: 10px; text-decoration: none;">
            ...
            <div id="content">
                ...
                <form id="form-collection">
                    <input type="hidden" id="salary_type" name="salary_type" value="Short">
                    <input type="hidden" id="instalment_type" name="instalment_type" value="Instalment">
                    <div>
                        <div class="label">
                            <span class="btn btn-secondary btn-salary-type" id="btn-short">Short</span>
                        </div>
                        <div class="input-group" >
                            <span class="input-group-text" style= "display:none;">RM</span>
                            <input type="hidden" class="form-control" id="salary" name="salary" min="0" step="0.1" required>
                        </div>
                    </div>
                    <div>
                        <div class="label label-instalment" style="display: flex; align-items: center;">
                            <div class="instalment-type-container">
                                <span class="btn btn-secondary btn-instalment-type">Instalment</span>
                                <!-- &nbsp;/&nbsp; -->
                    ...
                    <div style="display: none;">
                        <div class="label label-tepi2" style="display: flex; align-items: center;">
                            <span>Tepi 2</span>
                            <input type="month" class="form-control" id="tepi2_month" name="tepi2_month" value="<?php echo date('Y-m', strtotime(date('Y-m-01') . ' +1 month')); ?>">
                        </div>
                        <div class="input-group">
                            <span class="input-group-text">RM</span>
        ...
        <script>
            ...
            //             $('#instalment').val(response.loan_monthly ? response.loan_monthly : 0);
            //             $('#salary').val(0);
            //             $('#tepi1').val(0);
            //             $('#tepi2').val(0);
            //             $('#tepi2_bunga').val(0);
            ...
            //                 $('a[href^="../payment/add_monthly.php"]').attr('href', `../payment/add_monthly.php?id=${response.customer_id}`);
            //                 $('a[href^="../payment/add_instalment.php"]').attr('href', `../payment/add_instalment.php?id=${response.customer_id}`);
            //             }
            //         },
            //         error: function(xhr, status, error) {
            //             console.error(error);
            ...
            function search() {
                ...
                function trySecondURL() {
                    $.ajax({
                        url: 'get_monthly_customer_details_ajax.php',
                        type: 'POST',
                        data: requestData,
                        dataType: 'json',
                        success: function(response) {
                            if (response && Object.keys(response).length > 0) {
                                ...
                                console.warn('No customer data found in second attempt.');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error('Second AJAX call failed:', error);
                        }
                    });
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response && Object.keys(response).length > 0) {
                            ...
                            console.warn('No customer data found, trying fallback...');
                            trySecondURL();
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('First AJAX call failed, trying fallback:', error);
                        trySecondURL();
                    }
                });
            }
            ...
            function reset() {
                ...
                $('a[href^="../payment/add_monthly.php"]').attr('href', '../payment/add_monthly.php');
                $('a[href^="../payment/add_instalment.php"]').attr('href', '../payment/add_instalment.php');
            ...
            function goToCollection() {
                ...
                $('#collection').show(0, function () {
                    ...
                    if (!branch) {
                        console.warn('Branch cookie is empty.');
                        $('#collection-title').html(`Collection - UNKNOWN BRANCH`);
                        return;
                    }

                    $('#collection-title').html(`Collection - ${branch.toUpperCase()}`);

                    // Arrays to hold combined values
                    let loanCodeSet = new Set();
                    let customerCodeSet = new Set();

                    // Helper to fetch and merge from a given URL
                    function fetchCustomerList(url, callback) {
                        $.ajax({
                            url: url,
                            type: 'POST',
                            data: { branch: branch },
                            ...
                            success: function (response) {
                                if (response) {
                                    (response.loan_code || []).forEach(code => loanCodeSet.add(code));
                                    (response.customer_code || []).forEach(code => customerCodeSet.add(code));
                                }
                                callback(); // continue after fetching
                            },
                            error: function (xhr, status, error) {
                                console.error(`AJAX failed from ${url}:`, error);
                                callback(); // still continue even on error
                            }
                        });
                    }

                    // Final rendering after both calls
                    function renderCombinedList() {
                        let loanHtml = '';
                        loanCodeSet.forEach(code => {
                            loanHtml += `<option value="${code}">`;
                        });
                        $('#loan_codes').empty().html(loanHtml);

                        let customerHtml = '';
                        customerCodeSet.forEach(code => {
                            customerHtml += `<option value="${code}">`;
                        });
                        $('#customer_codes').empty().html(customerHtml);
                    }

                    // Sequential fetch from both sources
                    fetchCustomerList('get_customer_list_ajax.php', function () {
                        fetchCustomerList('get_monthly_customer_list_ajax.php', function () {
                            renderCombinedList(); // render after both are done
                        });
                    });
                });
            }
