...
$late_interest_q = mysql_query("SELECT * FROM late_interest_record WHERE id = '".$id."'") or die (mysql_error());
$late = mysql_fetch_assoc($late_interest_q);

$sql = mysql_query("SELECT * FROM customer_loanpackage WHERE customer_id = '".$late['customer_id']."'");
...
$loandetails_q = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$late['customer_id']."'");
...
$loan_scheme_q = mysql_query("SELECT * FROM loan_scheme WHERE id = '".$late['package_id']."'") or die (mysql_error());
$scheme = mysql_fetch_assoc($loan_scheme_q);
?>
...
<table width="1280" id="list_table">
    ...
    <tr>
      ...
      <td><input type="text" name="loan_package" value="<?php echo $scheme['scheme']; ?>" readonly="readonly" /></td>
    ...
    <tr>
      ...
      <td><input type="text" name="customer_code" id="customer_code" readonly="readonly" style="width:80px" value="<?php echo $late['loan_code']; ?>" /></td>
    ...
    <tr>
      ...
      <td style="padding:0px">Loan Amount (RM) </td>
        <td><input type="text" name="loan_amount" value="<?php echo "RM ".number_format($late['amount'], '2'); ?>" readonly="readonly" />&nbsp;<a href="payout.php?id=<?php echo $id; ?>" rel="shadowbox; width=500px; height=500px;"><img src="../img/customers/view-icon.png" /></a></td>
...
<table width="1280" id="list_table">
	...
	<tr <?php echo $style; ?>>
      ...
      <?php } else { echo date('d-m-Y', strtotime($get_l['payment_date'])); } ?>        </td>
   	  	<td><?php //echo $get_l['receipt_no']; ?><?php echo $late['loan_code']; ?></td>
...
<?php } else { ?>
		...
		<td>
			<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?><?php } else { ?>
			<?php //if($latectr == $latecount) { ?><?php //} else { echo "RM ".$get_l['payment']; } ?>
			<?php } ?>
	  <?php } else { echo "RM ".$get_l['payment_int']; } ?>		</td>
		<td>
			<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?><?php } else { ?>
			<?php //if($latectr == $latecount) { ?><?php //} else { echo $get_l['receipt_no']; } ?>
			<?php } ?>
	  <?php } else { echo $late['loan_code']; }//{ echo $get_l['receipt_no']; } ?>		</td>
		<td>
			<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?><?php } else { ?>
			<?php //if($latectr == $latecount) { ?><?php //}?>
			<?php } ?>
	  <?php } ?>		</td>
            <td>
				<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
				...
				?><?php } ?>
				<?php } ?>
				<?php } else { echo $get_l['month_receipt']; } ?>			</td>
            <td>
				<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
				<?php if($style == '') {?><?php } else { ?>
				<?php if($latectr == $latecount) { ?><?php  } else { ?><?php } ?>
				<?php } ?>
	  <?php } else { 
    ...
    <tr>
    	<td colspan="12" align="right"><input type="button" name="back" id="back" onClick="history.go(-1)" value=""></td>
    </tr>
    <tr>
    	<td colspan="12">&nbsp;</td>
    </tr>
