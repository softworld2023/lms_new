...
<table width="840" id="list_table" >
	...
	<?php
		...
		if ($login_branch == 'KTL' || $login_branch == 'TSY' || $login_branch == 'TSY2') {
			$branch_name_arr = array('MAJUSAMA', 'MAJUSAMA2', 'ANSENG', 'YUWANG', 'DK', 'KTL', 'TSY', 'TSY2');
			$db_name_arr = array('majusama', 'majusama2','anseng', 'yuwang', 'dk', 'ktl', 'tsy', 'tsy2');
		} else {
			$branch_name_arr = array('MAJUSAMA', 'MAJUSAMA2', 'ANSENG', 'YUWANG', 'DK');
		...
		for ($i = 0, $len = count($branch_name_arr); $i < $len; $i++) {
			...
			while ($result = mysql_fetch_assoc($q)) {
				...
				// var_dump("SELECT SUM(payout_amount) AS previous_monthly_amount, SUM(balance) AS previous_monthly_bd FROM " . $db_name . ".monthly_payment_record WHERE loan_code = '" . $result['loan_code'] . "' AND status = 'BAD DEBT'");
				...
				$query = mysql_query("SELECT MIN(balance) AS min_balance 
									...
									WHERE receipt_no = '" . $result['loan_code'] . "'
				...
				// var_dump("SELECT SUM(amount) AS collected FROM " . $db_name . ".late_interest_payment_details WHERE lid = '" . $late_interest_record_id . "'");
				while ($row = mysql_fetch_assoc($query)) {
					$previous_months_bd_collected += round($row['collected'],2);
				}
	
				
				$query = mysql_query("SELECT SUM(amount) AS collected, month_receipt FROM late_interest_payment_details WHERE lid = '" . $late_interest_record_id . "' AND month_receipt = '" . $payout_month . "'");
				while ($row = mysql_fetch_assoc($query)) {
				...
				// var_dump('1-'.$balance_without_instalment);
				// var_dump('2-'.$amount_without_instalment);
				// var_dump('3-'.$previous_monthly_bd);
				// var_dump('4-'.$envelope_without_instalment);
