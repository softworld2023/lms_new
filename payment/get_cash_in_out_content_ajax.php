<?php
    ...
    // Fetch saved tbody snapshot for a table+date.
    // If $fallbackPrev = true, and exact date not found, fallback to latest previous date.
    function fetch_saved_content($db, $tableNo, $date, $fallbackPrev = false) {
        $d  = esc($date);
        ...
        // 2) Optional fallback to latest previous date
        if ($fallbackPrev) {
            $sql2 = "SELECT content
                    FROM {$tn}
                    WHERE `date` < '{$d}'
                    ...
                    LIMIT 1";
            $q2 = mysql_query($sql2);
            if ($q2 && mysql_num_rows($q2) === 1) {
                $row2 = mysql_fetch_assoc($q2);
                $content2 = isset($row2['content']) ? $row2['content'] : '';
    ...
    if (in_array($table_no, array('1','4','7','8'), true)) {
        ...
        // Only tables 1,4,7 fallback to previous day
        $fallbackPrev = in_array($table_no, array('1','4','7'), true);

        $saved = fetch_saved_content($db, $table_no, $date, $fallbackPrev);
        if ($saved !== '') { echo $saved; exit; }

        // else render scaffold below…

    } elseif (in_array($table_no, array('2','5'), true)) {
        $saved = fetch_saved_content($db, $table_no, $date, false);
        if ($saved !== '') { echo $saved; exit; }
    }
    ...
    switch ($table_no) {
        ...
        //                 exit;
            ...
            while ($row = mysql_fetch_assoc($query)) {
                ...
                $m = DateTime::createFromFormat('Y-m-d', $row['month'] . '-01');
                $month_name = $m ? $m->format('M') : '';
                var_dump($row['month'] . ' to ' . $month_name);
        ...
        case '5':
            ...
            while ($row = mysql_fetch_assoc($query)) {
                ...
                $sm = $row['start_month'];
                if (strlen($sm) == 7)      $dt = DateTime::createFromFormat('Y-m-d', $sm . '-01');
