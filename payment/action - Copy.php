...
if(isset($_POST['adjust_loan']))
{
	...
	if($get_l['loan_period'] != $month)
	{
		...
		if($update_l)
		{
			$int_bal = $get_p['interest'] * $prev_balmonth;
			
			if($add_on != '' && $reduce == '')
			{
				$new_intbal = $int_bal + $add_on;
				$new_intpermonth = round($new_intbal / $bal_month);
				$new_totbal = $get_p['balance'] + $add_on;
				$loan_total = $get_l['loan_total'] + $add_on;
			}else			
			if($reduce != '' && $add_on == '')
			{
				$new_intbal = $int_bal - $reduce;
				$new_intpermonth = round($new_intbal / $bal_month);
			...
			$update_q = mysql_query("UPDATE loan_payment_details SET interest = '".$new_intpermonth."', balance = '".$new_totbal."' WHERE id = '".$payment_id."'");
			
			if($update_q)
			{
				$insert_q = mysql_query("INSERT INTO adjustment SET customer_loanid = '".$loan_id."', month = '".$month."', add_on = '".$add_on."', reduce = '".$reduce."', reason = '".$reason."'");
				if($insert_q)
				{
	...
	}else
	{
		...
		{
			$new_intbal = $int_bal + $add_on;
			$new_intpermonth = round($new_intbal / $prev_balmonth);
			$new_totbal = $get_p['balance'] + $add_on;
			$loan_total = $get_l['loan_total'] + $add_on;
		}
		...
		$update_q = mysql_query("UPDATE loan_payment_details SET balance = '".$new_totbal."', interest = '".$new_intpermonth."' WHERE id = '".$payment_id."'");
