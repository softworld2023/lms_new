		...
		if($loan_balance > 0)
		{
			...
			if($amount == 0)
			{
				...
				{			
					...
					$cashbook3_q = mysql_query("INSERT INTO cashbook SET package_id = '".$get_q['package_id']."', type = 'COMMISSION', amount = '".$int_amount."', transaction = 'KOMISYEN2', customer_id = '".$get_lp['customer_id']."', receipt_no = '".$receipt_no."', date = now()");
					
					//insert to balance2 record the interest
					$balance2_ql = mysql_query("INSERT INTO balance_rec SET package_id = '".$get_q['package_id']."', customer_loanid = '".$get_q['customer_loanid']."', received = '".$int_amount."', month_receipt = '".$month_receipt."', bal_date = now()");
				}
				
				//insert to cashbook record the interest
				$cashbook3_q = mysql_query("INSERT INTO cashbook SET package_id = '".$get_q['package_id']."', type = 'COMMISSION', amount = '".$int_amount."', transaction = 'KOMISYEN2', customer_id = '".$get_lp['customer_id']."', receipt_no = '".$receipt_no."', customer_id = '".$get_lp['customer_id']."', date = now()");
...
if($_POST['action'] == 'payloan')
{
	...
	if($ar == 0)
	{
		...
		if($update_q)
		{
			$cashbook_q = mysql_query("INSERT INTO cashbook SET type = 'RECEIVED', package_id = '".$get_q['package_id']."', table_id = '".$id."', transaction = 'REC', code = '".$get_lp['loan_code']."', receipt_no = '".$get_q['receipt_no']."', customer_id = '".$get_lp['customer_id']."', amount = '".$amount."', date = now()");
	...
	}else
	{
		...
		if($update_q)
		{
			$cashbook_q = mysql_query("INSERT INTO cashbook SET type = 'RECEIVED', package_id = '".$get_q['package_id']."', table_id = '".$id."', transaction = 'REC', code = '".$get_lp['loan_code']."', amount = '".$amount."', customer_id = '".$get_lp['customer_id']."', receipt_no = '".$get_q['receipt_no']."', date = now()");
