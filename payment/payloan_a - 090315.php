...
<table width="1280" id="list_table">
    ...
    <tr>
   	  <td align="right" style="padding:0px">Code</td>
        <td><input type="text" name="customer_code" id="customer_code" readonly="readonly" style="width:100px" value="<?php echo $get_q['loan_code']; ?>" /></td>
    ...
    <tr>
      ...
      <td align="right" style="padding:0px">Loan Period</td>
      <td><input type="text" name="period" id="period" value="<?php echo $get_q['loan_period']; ?>" style="width:50px" onkeyup="changePeriod(this.value,'<?php echo $get_q['loan_period']; ?>')" /> months <div id="save_period" style="display:none"><a href="javascript:updatePeriod('<?php echo $get_q['loan_period'] ?>', '<?php echo $id; ?>')"><input type="button" name="saveperiod" id="saveperiod" value="" /></a></div></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td align="right" style="padding:0px">Remarks</td>
        <td><textarea style="width:230px" readonly="readonly"><?php echo $get_q['loan_remarks']; ?></textarea></td>
	...
	<tr>
		...
		<td><input type="text" name="bankacc" id="bankacc" value="<?php echo $getacc['a_bankbranch']; ?>" style="width:250px" /></td>
		<td align="right">Pay Day</td>
		<td><input type="text" name="payday" id="payday" value="<?php echo $getacc['a_payday']; ?>" style="width:230px" /></td>
		<td></td>
		<td></td>
	</tr>   
    <tr>
...
<table width="1280" id="list_table">
    ...
    <tr <?php echo $style; ?>>
      	...
      	<td><?php if($get_l['next_paymentdate'] != '0000-00-00') { echo $get_l['next_paymentdate']; } else { echo '-'; } ?></td>
  	  	...
  	  	<td><?php echo $get_l['int_percent']." %"; ?></td>
    	<td><?php echo "RM ".$get_l['monthly']; ?></td>
      	<td>
       
		  <?php if($get_l['payment'] == '0' && $get_l['payment_date'] == '0000-00-00') {  ?>
		  <?php if($style == '') {?>
		  <input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" /><input type="text" name="payment" id="payment" class="currency" style="width:130px" placeholder="RM <?php echo $get_l['monthly']; ?>" autocomplete="off" />
		  <?php } else { ?>
        ...
        <td>
			...
			<?php } ?>
			<?php } ?>
			<?php } else { echo $get_l['payment_date']; } ?>  
        ...
        <td>
			...
			<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?>
			<input type="text" name="new_receipt" id="new_receipt" style="width:100px" value="<?php echo $get_q['loan_code']; ?>" autocomplete="off"/>
			<?php } else { ?>
			<?php if($latectr == $latecount) { ?>
			<input type="text" name="new_receipt" id="new_receipt" style="width:100px" value="<?php echo $get_q['loan_code']; ?>" autocomplete="off"/>
			<?php } else { ?>
			...
			<?php } ?>
			<?php } ?>
			<?php } else { echo $get_l['loan_code']; } ?> 
			
			</td>
        <td>
		<?php if($get_l['payment_date'] == '0000-00-00') {  ?>
			<?php if($style == '') {?>
			<input type="text" name="next_paymentdate" id="next_paymentdate" style="width:130px" />
			<?php } else { ?>
			<?php if($latectr == $latecount) { ?>
			<input type="text" name="next_paymentdate" id="next_paymentdate" style="width:130px" />
			<?php } else { ?>
        ...
        <td>
			<?php if($get_l['payment'] == '0' && $get_l['payment_date'] == '0000-00-00' && $ctr == $ldrow) { ?>
            <a href="javascript:payConfirm('<?php echo $get_q['loan_period']; ?>', '<?php echo $get_l['month']; ?>')" title="pay"><img src="../img/payment-received/payment-received.png" width="30" /></a>&nbsp;&nbsp;&nbsp;<?php if($style == '') { ?><a href="adjust.php?id=<?php echo $get_l['id']; ?>&p=<?php echo $get_q['loan_period']; ?>" rel="shadowbox; width=800px; height=380px" title="adjust"><img src="../img/customers/edit-icon.png" /></a>
			<?php } else { ?>
			<a href="adjustlate.php?id=<?php echo $get_l['id']; ?>&p=<?php echo $get_q['loan_period']; ?>" rel="shadowbox; width=800px; height=380px" title="adjust"><img src="../img/customers/edit-icon.png" /></a>
			<?php } ?>
            <?php } ?>    	</td>
    ...
    <tr>
    	<td colspan="11" align="right"><input type="button" name="back" id="back" onClick="window.location.href='../payment/'" value=""></td>
    </tr>
    <tr>
    	<td colspan="11">&nbsp;</td>
    </tr>
