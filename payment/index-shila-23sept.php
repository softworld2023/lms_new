...
if(isset($_POST['search']))
{
	...
	//{
		...
		if($_POST['outdate'] != '')
		{
			...
			$cond .= " and pd.payment_date <= '".$outdate."' and (lp.loan_status = 'Paid') AND (lp.payout_date <= '".$outdate."' OR lp.payout_date = '0000-00-00')";
		}else
		{
...
<?php echo $_SESSION['cdate'] . "|" . $_SESSION['pmonth'] . "|" . $_SESSION['loanpackage'] .  "|" . $bal2checkrow . "|" .  $loancheckrow; ?>
<?php if($_SESSION['cdate'] != '' && $_SESSION['pmonth'] != '' && $_SESSION['loanpackage'] != '' && $bal2checkrow == $loancheckrow) { echo "satu"; ?>
...
<table width="1280" id="list_table">
	...
	//loan outstanding
	date('Y-m-d', strtotime($_POST['outdate']));
		...
		{//flexi receipt receipt type == 2 
			...
			if($_SESSION['pmonth'] != '') {		
				...
				{ $outbal1 = 'outball = on';
					...
					{$outloanreceived = 'outloanreceived = on';
						//get opening  from loan_payment_details
						if($outbalance < 0)
						{ $outloanreceived = 'out<0=on';
			...
			{ $other = 'other=on';
				...
				{		$loanpackage1 = 'loanpackage date 0 = on';	
					//get from balance 1
				...
				{ $loanpackage2 = 'loanpackage date ! 0 = on';	
					//get from balance 1
					$outb_q = mysql_query("SELECT SUM(loan) AS loan, SUM(received) AS received FROM balance_transaction WHERE customer_loanid = '".$get_q['id']."' AND date <= '".$_SESSION['cdate']."'");
					$outb = mysql_fetch_assoc($outb_q);
