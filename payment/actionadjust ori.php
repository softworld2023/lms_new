...
if(isset($_POST['adjust_late']))
{
	...
	for($i = $curr_month+1; $i<= $ctrmth; $i++)
	{
		if($i != $ctrmth)
		{
			...
			$npd = $y.'-'.$m.'-'.$d;
			$insert = mysql_query("INSERT INTO loan_payment_details SET customer_loanid = '".$get_c['customer_loanid']."', month = '".$i."', int_percent = '".$get_c['int_percent']."', monthly = '".$get_c['monthly']."', balance = '".$get_c['balance']."', int_balance = '".$get_c['int_balance']."', pokok = '".$get_c['pokok']."', interest = '".$get_c['interest']."', package_id = '".$get_c['package_id']."', receipt_no = '".$get_c['receipt_no']."', next_paymentdate = '".$npd."'");
		...
		else
		{
			...
			$insert = mysql_query("INSERT INTO loan_payment_details SET customer_loanid = '".$get_c['customer_loanid']."', month = '".$i."', int_percent = '".$get_c['int_percent']."', monthly = '".$monthly."', balance = '".$balance."', int_balance = '".$get_c['int_balance']."', pokok = '".$get_c['pokok']."', interest = '".$interest."', package_id = '".$get_c['package_id']."', receipt_no = '".$get_c['receipt_no']."', next_paymentdate = now()");
				...
				if($update)
				{
					if($add_on != '')
					{
						...
						if($insert_a)
						{
							...
							$balancerec_q = mysql_query("INSERT INTO balance_rec SET package_id = '".$get_c['package_id']."', customer_loanid = '".$get_q['customer_loanid']."', interest = '".$add_on."', bal_date = '".$get_c['next_paymentdate']."'");
