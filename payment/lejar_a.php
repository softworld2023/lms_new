...
$sql2 = mysql_query("SELECT no_resit, balance FROM loan_lejar_details WHERE receipt_no = '".$get_q['loan_code']."' ORDER BY id ASC LIMIT 1");
$result2 = mysql_fetch_assoc($sql2);

$balance = $result2['balance'];
$loan_faedah = $balance - $get_q['loan_pokok'];

if($result2['no_resit']!='')
{
	$no_resit = $result2['no_resit'];
}
else
{
...
<!--       	<?php if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { ?>
    ...
    <tr>
      ...
      <td style="padding:0px">Jumlah Besar</td>
        <td><input type="text" name="loan_amount" value="<?php echo "RM ".number_format($balance, '2'); ?>" readonly="readonly" /></td>
...
<table width="1280" id="list_table">
	...
	$latecount_q = mysql_query("SELECT * FROM loan_lejar_details WHERE receipt_no = '".$get_q['loan_code']."' AND payment_date = '0000-00-00'");
	$latecount = mysql_num_rows($latecount_q);

	$q = $branch == 'KTL' ? $loandetails_q : $latecount_q;
	
	while($get_l = mysql_fetch_assoc($q))
	{
	$ctr++;
...
{ echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } else { ?><input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } ?>" /><a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save schedule date"><img src="../img/document_save.png" /></a><?php } ?></td> -->
		<!---payment date-->
          ...
          <?php } ?>
<td>
  	<input type="text" name="no_resit_<?php echo $ctr; ?>" id="no_resit_<?php echo $ctr; ?>" value="<?php echo $no_resit+$ctr-1;?>" style="width:100px"/>
        ...
        <td><?php 
			if($get_l['payment'] == '0' && $get_l['payment_date'] == '0000-00-00' ) 
			{ ?>
				<a href="javascript:payConfirm('<?php echo $get_q['loan_period']; ?>', '<?php echo $get_l['month']; ?>')" title="pay"><img src="../img/payment-received/payment-received.png" width="30" /></a><a href="adjustment.php?id=<?php echo $get_q['id']; ?>" rel="shadowbox; width=600px; height=420px"><img src="../img/customers/settle.png" width="23px;" height="23px;" title="Settlement"></a>
				<a href="blacklist.php?id=<?php echo $get_q['id']; ?>" rel="shadowbox; width=600px; height=400px"><img src="../img/error.png" title="Bad Debt" width="25"></a><?php 
			}
			else 
			{ ?><?php 
				...
				{ ?><?php 
					...
					if($checkrow == 1)
					{?><a href="payment_receipt.php?id=<?php echo $get_l['id']; ?>" target="_blank"><img src="../img/customers/view-icon.png" alt="" title="View Payment Receipt" /></a>&nbsp;&nbsp;&nbsp;
   ...
   <?php }else if($get_l['bad_debt']=="yes"){
?>
    <tr>
...
function payConfirm(period, month)
{
	...
	else
	{
		...
		$no_resit = document.getElementById('no_resit_'+month).value;
	
		$period = period;
		$currmonth = month;
		
		if($amount == $paymentmonthly && $date != '' && $nextdate != '' && $receipt != '')
		{
			$.confirm({
