...
<table width="1280" id="list_table">
    ...
    <tr>
      <td style="padding:0px"><span style="padding:0px; padding-top:5px">Notes <?php if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) { ?>
...
<table width="1280" id="list_table">
	...
	{//this means the it's not reaching due date yet
		$style = '';
	}else
	{
	...
	?>
    <?php if(($get_l['balance'] != 0) || ($get_l['payment'] != 0) ) { $ccm++; ?>
	
    <tr <?php echo $style; echo $get_l['id']; ?>>
      	<td><?php echo $ctr; ?></td>
      	<td>
		...
		<?php
			...
			<?php
				...
				<input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } ?>" /><a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save schedule datezzz"><img src="../img/document_save.png" /></a><?php 
			} ?></td>
		...
		if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff'  || $_SESSION['login_username'] == 'huizhen')) { ?>
			<?php $checkrow_q = mysql_query("SELECT * FROM loan_payment_details WHERE id > '".$get_l['id']."' AND customer_loanid = '".$id."' LIMIT 1"); 
			$checkrow = mysql_fetch_assoc($checkrow_q);
		
			if($checkrow['payment_date'] == '0000-00-00' || $get_l['payment_date'] != '0000-00-00')
			{
			?>
			<a href="javascript:deletepayConfirm2('<?php echo $id; ?>', '<?php echo $get_l['id']; ?>', '<?php echo date('d-m-Y', strtotime($get_l['payment_date'])); ?>','<?php echo $get_q['loan_total']; ?>')" title="delete"><img src="../img/delete-btn.png" width="31" height="25" /></a>
			<?php } //end of $chekcrow ?>
			<?php } ?>
	...
	<tr style="color:#0000FF; font-weight:bold">
		...
		<td><input type="hidden" id="lid" name="lid" value="<?php echo $id; ?>" />
         ...
         <?php 
			if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff'  || $_SESSION['login_username'] == 'huizhen')) { ?>
			<?php 
			$checkrow_q3 = mysql_query("SELECT * FROM loan_payment_details WHERE id > '".$get_l['id']."' AND customer_loanid = '".$id."'"); 
							$checkrow3 = mysql_num_rows($checkrow_q3);
							if($checkrow3 == 5)
							{
						?>			            
								<a href="javascript:deletepayConfirm2('<?php echo $id; ?>', '<?php echo $get_l['id']; ?>', '<?php echo date('d-m-Y', strtotime($get_l['payment_date'])); ?>','<?php echo $get_q['loan_total']; ?>')" title="delete"><img src="../img/delete-btn.png" width="31" height="25" /></a>
...
<table width="1280" id="list_table">
	<tr>
	  	...
	  	<th>Receipt No </th>
	  	<th>Balance</th>
	  	<th>Monthly Amountkkkkk <!--<a href="edit_monthlyamount.php?id=<?php echo $id;?>" rel="shadowbox; width=800px; height=380px" title="Edit Monthly Amount" style="color:#00CCFF">(Edit)</a>--></th>
      	<th>Payment Received</th>
        <th>Payment Date</th>
        <th>&nbsp;</th>
        <th width="90">&nbsp;</th>
    ...
    <?php
	// --- testing part ---
		$dt = strtotime("2018-01-31");
		echo date("Y-m-d", $dt);
		echo ' -- BECOME --> ';
		//echo date ("Y-m-d",strtotime("last day of +1 month", $dt));
		$new_monthpork = date ("Y-m-d",strtotime("last day of +1 month", $dt));
		echo $new_monthpork;
	// --- testing part ---
    ...
    <tr rowspan = '2' <?php echo $style; ?>>
		...
		<p>Lalalalala</p>
      	<td><?php 
			if($_SESSION['login_username'] != 'softworld' && $_SESSION['login_username'] != 'fong' && $_SESSION['login_username'] != 'huizhen' && $_SESSION['login_username'] != 'staff') { 
				echo date('d-m-Y', strtotime($get_l['next_paymentdate']));
			?>
				<input type="hidden" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" 
					style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') 
												{
													echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); 
												}// ** STAFF GOING TO HERE! ?>"/>
			<?php
			} else {?>
				<input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" 
					style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') 
												{ 
													echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); 
												}// **  BOSS GOING TO HERE! ?>"/>
				<!-- The black diskett icon -->
				<a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" 
					title="save schedule date"><img src="../img/document_save.png" /></a><?php 
			} ?>
		</td>
		
   	  	<td><?php echo $get_l['receipt_no']; ?><input type="hidden" name="prev_receipt" id="prev_receipt" value="<?php echo $get_l['receipt_no'] ?>" /></td>
   	  	<td><?php echo "RM ".$get_l['balance']; ?></td>
  	  	<td><?php echo 'John Cena - ';echo "RM ".$get_l['monthly']; ?></td>
        ...
        <td>
			<?php if($get_l['payment_date'] != '0000-00-00'){ if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) 
			{ ?> <input type="text" name="payment_date_<?php echo $ctr; ?>" id="payment_date_<?php echo $ctr; ?>" style="width:80px" value="<?php echo date('d-m-Y', strtotime($get_l['payment_date']));  ?>">   <a href="javascript:edit_pdate('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save payment date"><img src="../img/document_save.png" /></a> <?php } else { echo $get_l['payment_date']; }} else {?>
			<?php if($hnp == 1) { ?>
			<input type="text" name="payment_date" id="payment_date" style="width:130px" value="<?php echo date('Y-m-d'); ?>" />
			<?php } ?> 
			<?php } ?>
		...
		<td>
			<?php if($get_l['payment_date'] == '0000-00-00'){?>
			<?php if($hnp == 1) { echo 'TEST TEST muahahahahaha';?>
			<input type="hidden" name="new_receipt" id="new_receipt" style="width:100px" value="<?php echo $get_q['loan_code']; ?>"/>
	  ...
	  <?php } ?>		</td>
        <td>
			...
			<?php if($get_l['payment'] == '0' && $get_l['payment_date'] == '0000-00-00') { ?>
			<?php if($hnp == 1) { ?>
				<a href="javascript:payConfirm('<?php echo $get_q['loan_period']; ?>', '<?php echo $get_l['month']; ?>')" title="payzzzzz"><img src="../img/payment-received/payment-received.png" width="30" /></a>&nbsp;&nbsp;&nbsp;
			<?php
				//this is the pencil button beside the cash logo there. Only Boss can view it!
				if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) 
			{?>
				<a href="adjust.php?id=<?php echo $get_l['id']; ?>&p=<?php echo $get_q['loan_period']; ?>" rel="shadowbox; width=800px; height=380px" title="adjust"><img src="../img/customers/edit-icon.png" /></a>
        <?php 
			} 
			}
					?>
   ...
   <?php if($bal_cur != 0) {?>
	<tr style="color:#0000FF; font-weight:bold">
		...
		<td><input type="hidden" id="lid" name="lid" value="<?php echo $id; ?>" />
			<a href="javascript:payCCM2('<?php echo $ctr; ?>')" title="pay laaaaaaa"><img src="../img/payment-received/payment-received.png" width="30" /></a>		</td>
	</tr>
	<?php } ?>
    <tr>
    	<td colspan="9">&nbsp;</td>
...
function payCCM2(ctr)//for loan that pay 1 cheque 
{
	$ctr = ctr;
	$amount = document.getElementById('ccm_int').value;
	$date = document.getElementById('ccm_date').value;
	$ptype =  document.getElementById('ptype').value;
	$date2 = document.getElementById('edit_npd_'+$ctr).value;
	//$nextdate = document.getElementById('next_paymentdate').value;
	$id = document.getElementById('lid').value;
	//$receipt = document.getElementById('new_receipt').value;
	//$prev_receipt = document.getElementById('prev_receipt').value;
...
function deletepayConfirm2(loanid, id, date,amount,package_id)
{
	$id = id;
	$loanid = loanid;
	$amount = amount;
