...
<table width="1280" id="list_table">
	...
	<tr <?php echo $style; ?>>
      	<td><?php echo $ctr; ?></td>
      	<td><?php echo $get_l['next_paymentdate']; ?></td>
   	  	<td><?php echo $get_l['receipt_no']; ?></td>
   	  	<td><?php echo "RM ".$get_l['balance']; ?></td>
   	  	<td><?php echo $get_l['int_percent']." %"; ?></td>
        <?php if($todaydate <= $get_l['next_paymentdate']){ ?>
            <td><?php $totalp = number_format(($get_l['pokok'] + $get_l['interest']), '2'); ?>
                <?php if($get_l['payment_date'] == '0000-00-00') {  ?><input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" /><input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM" /><?php } else { echo "RM ".$get_l['payment']; } ?>            </td>
            <td>
                <?php if($get_l['payment_date'] == '0000-00-00') {  ?><input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM" /><?php } else { echo "RM ".$get_l['payment_int']; } ?>            </td>
            <td>
                <?php if($get_l['payment_date'] != '0000-00-00'){ echo $get_l['payment_date']; } else {?><input type="text" name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('Y-m-d'); ?>" /><?php } ?>			</td>
            ...
            <td>
				<?php if($get_l['payment_date'] == '0000-00-00'){?>
					<input type="text" name="new_loan" id="new_loan" style="width:80px" class="currency"/><input type="hidden" name="month_receipt" id="month_receipt" style="width:80px" value="<?php echo date('Y-m'); ?>"/>
				<?php } ?>			</td>
            <td><?php if($get_l['payment_date'] != '0000-00-00'){
            ...
            {
            ?>
                <input type="text" name="next_paymentdate" id="next_paymentdate" style="width:80px" />
                <?php } ?></td>
            <td>
		...
		?>
        	<td>
				<?php echo "RM ".$get_l['payment']; ?>            </td>
            <td>
                <?php echo "RM ".$get_l['payment_int']; ?>            </td>
            <td>
                <?php if($get_l['payment_date'] != '0000-00-00'){ echo $get_l['payment_date']; } else { ?>-<?php } ?>            </td>
            <td>
			<?php 
			if($get_l['payment_date'] != '0000-00-00'){
				echo $get_l['receipt_no'];
            ...
            {
				?>
            	<a href="adjustlateflexi.php?id=<?php echo $get_l['id']; ?>&p=<?php echo $get_q['loan_period']; ?>" rel="shadowbox; width=800px; height=380px" title="adjust"><img src="../img/customers/edit-icon.png" /></a>
            <?php } } ?>    	</td>
        <?php
		}
		?>
    ...
    <tr>
    	<td colspan="12" align="right"><input type="button" name="back" id="back" onClick="window.location.href='../payment/'" value=""></td>
    </tr>
    <tr>
    	<td colspan="12">&nbsp;</td>
    </tr>
