...
if(isset($_POST['search']))
{
	if($_POST['month'] != '')
	{
		...
		$statement = "`loan_payment_details` pd, `customer_details` cd, `customer_loanpackage` lp WHERE lp.loan_status = 'Paid' AND lp.id = pd.customer_loanid AND lp.branch_id = '".$_SESSION['login_branchid']."' AND lp.customer_id = cd.id $cond ORDER BY cd.name ASC";
	...
	}else
	{
		...
		$statement = "`customer_details` cd, `customer_loanpackage` lp WHERE lp.loan_status = 'Paid' AND lp.branch_id = '".$_SESSION['login_branchid']."' AND lp.customer_id = cd.id $cond ORDER BY cd.name ASC";
		$_SESSION['payment_s'] = $statement;
	}
}
else
