<?php
    ...
    //             status
    //         FROM
    //             $db.monthly_payment_record
    //         WHERE month = '$selected_year-$selected_month' AND status != 'DELETED'
    //         GROUP BY
    //             loan_code ASC
    //         ORDER BY
    //             monthly_date ASC, id ASC";
    // // var_dump($sql);
    ...
    //     $return = $row['PA'] - $row['balance'];
    //     $payout = $row['PA'] - ($row['PA'] * 0.1);
    //     // echo $return . ' ' . $payout . '<br>';
    //     $total_return += $return;
    //     $total_payout += $payout;
    // }
    // var_dump($total_return);
    // var_dump($total_payout);
    // $closing_balance = $total_return - $total_payout;
    // var_dump($closing_balance);
    ...
    $initial_closing_balance_monthly = isset($res['initial_closing_balance_monthly']) ? $res['initial_closing_balance_monthly'] : 0;
...
.customBtn:active {
  ...
  .customBtn a {
   ...
   #tbl-account-book-monthly {
		border-collapse:collapse;
		border:none;	
	}

	#tbl-account-book-monthly tr th {
		height:36px;
		background:#666;
		text-align:left;
		padding-left:10px;
	...
	#tbl-account-book-monthly tr td {
		height:35px;
		padding-left:10px;
		padding-right:10px;
	}
...
@media print {
    ...
    #tbl-account-book-monthly tr {
        page-break-inside: avoid;
    }

    /* Make table header repeat on each page */
    #tbl-account-book-monthly thead {
        display: table-header-group;
    }

    #tbl-account-book-monthly tfoot {
...
<table width="1280">
        	...
        	<table>
				<tr>
                    ...
                    <td style="padding-right:8px">
                    	<input type="button" id="search" name="search" value="" onclick="searchAccountBookMonthly();"/>
					</td>
                </tr>
	...
	<tr>
        ...
        <div class="subnav">
            ...
            <a href="account_book_monthly.php" id="active-menu">Account Book (Monthly)</a>
...
<table width="1280" id="list_table">
             ...
             <table width="100%" border='0'>
                 <tr>              
                     <td style="font-size: 16px;"><br>YEAR
                         ...
                         <input class="btn btn-blue" type="button" id="btn-search-account-book-monthly" name="search_return_book_monthly" value="Search" style="width: 100px; height: 30px; font-size:16px;" onclick="searchYearMonth();">
                     </td>
 ...
 <div id="initial-closing-balance-container">
    ...
    <div>
        <input type="number" id="initial_closing_balance" name="initial_closing_balance" style="height: 30px; width: 100px;" value="<?php echo $initial_closing_balance_monthly; ?>"/>
    </div>
...
<div id="print-only-header">
    <div style="font-size: 20px; font-weight: bold;">ACCOUNT BOOK (MONTHLY)</div>
    <div id="print-only-date" style="margin-top: 5px; font-size: 16px;"></div>
</div>
 <table width="1280" id="tbl-account-book-monthly">
    <thead>
...
<script>
    ...
    $('#btn-save').on('click', function() {
        $.ajax({
            url: 'update_account_book_monthly_ajax.php',
            type: 'POST',
            data: {
    ...
    function searchAccountBookMonthly() {
        $.ajax({
            url: 'get_account_book_monthly_listing_ajax.php',
            type: 'POST',
            data: {
                year: $('#selected_year').val(),
                month: $('#selected_month').val(),
            ...
            success: function(response) {
                // console.log(response);
                $('#tbl-account-book-monthly tbody').html(response);
            }
        });
    }
    ...
    function searchYearMonth() {
        ...
        window.location.href = 'account_book_monthly.php?year=' + year + '&month=' + month;
    }
</script>