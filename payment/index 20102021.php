...
if(isset($_POST['search']))   // click search
{
	$cond = '';
		if($_POST['month'] != '')
		{
			$cond .= " and pd.month_receipt = '".$_POST['month']."' 
						and pd.payment_date = '0000-00-00'";
			...
			for($i=0; $i<sizeof($idList); $i++)
			{
				...
				{//however we fall in this category lol
					...
					$cond .= " or lp.customer_id = '".$idList[$i]."'
								AND (lp.loan_status = 'Paid' 
								OR lp.loan_status = 'Finished') ";
				}
			}
		...
		if($_POST['outdate'] != '')
		{
			...
			$cond .= " and pd.payment_date < '".$outdate."' 
						AND (lp.payout_date <= '".$_SESSION['cdate']."' OR lp.payout_date = '0000-00-00')
						AND (lp.loan_status = 'Paid' OR lp.loan_status = 'Finished') 
						";
		...
		}else
		{
			...
			$cond .= " and (lp.loan_status = 'Paid' OR lp.loan_status = 'Finished')";
			$_SESSION['cdate'] = '';
			$ccond .= '';
		}
		
		$statement = "`loan_payment_details` pd, `customer_loanpackage` lp 
						WHERE lp.id = pd.customer_loanid
						AND lp.branch_id = '".$_SESSION['login_branchid']."' $cond 
						GROUP BY lp.id 
						ORDER BY SUBSTRING_INDEX(lp.loan_code, IF(LOCATE(' ', lp.loan_code), ' ', '*'), 1) ASC, 
						SUBSTRING_INDEX(lp.loan_code, IF(LOCATE(' ', lp.loan_code), ' ', '*'), -1)+0 ASC";
...
{
	$statement = "`customer_loanpackage` WHERE (loan_status = 'Paid' OR loan_status = 'Finished')
					AND branch_id = '".$_SESSION['login_branchid']."' $cond 
					ORDER BY SUBSTRING_INDEX(loan_code, IF(LOCATE(' ', loan_code), ' ', '*'), 1) ASC, 
					SUBSTRING_INDEX(loan_code, IF(LOCATE(' ', loan_code), ' ', '*'), -1)+0 ASC";
}
       	...
       	<form action="index.php" method="post">
        	<table>
				<tr>
                    ...
                    <td  style="padding-right:30px"><input type="text" name="outdate" id="outdate" style="width:100px; height:30px" 
						value="<?php if($_SESSION['cdate'] != '') { echo date('d-m-Y', strtotime($_SESSION['cdate'])); } ?>" /></td>
...
<table width="1280" id="list_table">
    ...
    <tr <?php echo $style; ?>>
    	...
    	<td><?php echo $get_q['loan_code'];//$get_q['receipt_no']; ?></td>
