...
if($_POST['action'] == 'payloanf2') //flexi tapi resit guna code yang sama
{
	...
	if($new_loan == '' || $new_loan == 0)
	{
		...
		if($loan_balance > 0)
		{
			...
			if($amount == 0)
			{
				...
				$nxmr_q = mysql_query("SELECT * FROM loan_payment_details 
				...
				if(!$nxmr)
				{
					...
					$nm = $get_q['month'] + 1;
					$insert_lp_q = mysql_query("INSERT INTO loan_payment_details 
				...
				else
				{
					...
					$insert_lp_q = mysql_query("UPDATE loan_payment_details SET 
			...
			}else
			{
				...
				//check next month rec
				$nxmr_q = mysql_query("SELECT * FROM loan_payment_details WHERE 
										id > '".$id."' AND 
										customer_loanid = '".$get_q['customer_loanid']."'");							
				$nxmr = mysql_fetch_assoc($nxmr_q);
				...
				if(!$nxmr)
				{
					//insert new loan_payment_details
					$nm = $get_q['month'] + 1;
					$insert_lp_q = mysql_query("INSERT INTO loan_payment_details SET 
													customer_loanid = '".$get_q['customer_loanid']."', 
													month = '".$nm."', 
													int_percent = '".$get_q['int_percent']."', 
													balance = '".$loan_balance."',
	...
	{	
		...
		if($amount != 0)
		{
			...
			{
				$insert_lp_q = mysql_query("UPDATE loan_payment_details 
...
if($_POST['action'] == 'delete_lateint'){
	...
	$delete_q = mysql_query("DELETE FROM late_interest_payment_details 
		...
		$sql = "SELECT * FROM loan_payment_details WHERE receipt_no = '". $lir['loan_code'] . "' AND month_receipt = '". $lipd['month_receipt'] . "'";
        $q = mysql_query($sql);
        if ($q) {
			...
			if (!empty($lipd['collection_id'])) {
				$delete_collection_q = mysql_query("DELETE FROM collection WHERE id = '".$lipd['collection_id']."'") or die("Failed to delete collection: " . mysql_error());

				$sql = "UPDATE loan_payment_details SET month_receipt = '', payment = '', payment_date = '',loan_status = '' WHERE id = '$loan_payment_details_id'";
                mysql_query($sql);
    
                // delete from loan_payment_details	(next payment)
                $sql = "DELETE FROM loan_payment_details WHERE id > '$loan_payment_details_id' AND customer_loanid = '$loan_id'";
                mysql_query($sql);

				$sql = "DELETE FROM return_book_monthly WHERE collection_id = '".$lipd['collection_id']."'";
				mysql_query($sql);

				$sql = "DELETE FROM return_book_instalment WHERE collection_id = '".$lipd['collection_id']."'";
				mysql_query($sql);
			}
		}
		if($delete_q)
		{
			...
			$delete_collection = mysql_query("DELETE FROM collection WHERE id = '".$lipd['collection_id']."'");
...
if($_POST['action'] == 'delete_short'){
	...
	$short_q = mysql_query("SELECT * FROM short_details WHERE id = '".$id."'");
	$short = mysql_fetch_assoc($short_q);

	//short_details details
	$shortr_q = mysql_query("SELECT * FROM short_record WHERE id = '".$short_id."'");
	$short_record = mysql_fetch_assoc($shortr_q);

	$delete_amount = $short['amount'];
	$current_balance = $short_record['balance'];
	$new_balance = $current_balance + $delete_amount;

	$delete_b2q = mysql_query("DELETE FROM short_details 
								WHERE id = '$id' ");
	if($delete_b2q)
{
		//update late interest record
		...
		$_SESSION['msg'] = "<div class='success'>Short has been successfully deleted from the database.</div>";
...
if($_POST['action'] == 'payloan')
{
	...
	if($ar == 0)
	{
		$update_q = mysql_query("UPDATE loan_payment_details SET 
									payment = '".$amount."', 
									payment_date = '".$date."', 
									month_receipt = '".$month_receipt."', 
									loan_status = 'COLLECTED',
									staff_id = '".$_SESSION['taplogin_id']."', 
									staff_name = '".$_SESSION['login_name']."', 
									created_date = now() 
									WHERE id = '".$id."'");
		...
		if($update_q)
		{
			...
			/*if($num_select == '1')
			{*/
				$balance2_q = mysql_query("INSERT INTO balance_rec SET 
											package_id = '".$get_q['package_id']."', 
											customer_loanid = '".$get_q['customer_loanid']."', 
											received = '".$amount."', 
											bal_date = '".$date."', 
				...
				$balance1_q = mysql_query("INSERT INTO balance_transaction SET 
											package_id = '".$get_q['package_id']."', 
											customer_loanid = '".$get_q['customer_loanid']."', 
											received = '".$amount."', 
											date = '".$date."', 
			...
			//check next record 
			$nextp_q = mysql_query("SELECT * FROM loan_payment_details WHERE 
									customer_loanid = '".$get_q['customer_loanid']."' 
									AND id > '".$id."' ");
			...
			$newmonth = $month + 1;if(!$nextp)
			{
				...
				//{
					...
					{
						$newloanpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.9)+2));
						$newreturnpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.9)+2));

					}else if($get_lp['loan_period']>=13 && $get_lp['loan_period']<=24)
					{
						$newloanpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.92)+2));
						$newreturnpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.92)+2));

					}else if($get_lp['loan_period']>=25 && $get_lp['loan_period']<=36)
					{
						$newloanpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.938)+2));
						$newreturnpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.938)+2));

					}else if($get_lp['loan_period']>=37 && $get_lp['loan_period']<=48)
					{
						$newloanpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.945)+2));
						$newreturnpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.945)+2));
					}else
					{
						$newloanpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.95)+2));
						$newreturnpercent = round(((($get_q['loan_percent']-$get_mq['monthly'])/0.95)+2));
					}
					
					
					if($newbalance > 0)
					{
						$insert_new = mysql_query("INSERT INTO loan_payment_details SET 
													customer_loanid = '".$get_q['customer_loanid']."', 
													month = '".$newmonth."', 
													int_percent = '".$get_q['int_percent']."', 
													balance = '".$newbalance."', 
													...
													package_id = '".$get_q['package_id']."', 
													receipt_no = '".$receipt_no."',
													loan_percent = '".round($newloanpercent,2)."',
													return_percent = '".round($newreturnpercent,2)."', 
													branch_id = '".$_SESSION['login_branchid']."', 
													branch_name = '".$_SESSION['login_branch']."'");
			...
			}else
			{
				...
				// update balance
				$insert_new = mysql_query("UPDATE loan_payment_details SET 
											balance = '".$newbalance."', 
											monthly = '".$get_mq['monthly']."', 
											pokok = '".$pokok."', 
											interest = '".$get_q['interest']."', 
...
{  // --- get from payloan_CEK ---
	...
	$sql = mysql_query("SELECT * FROM loan_payment_details WHERE 
							id = '".$id."'");
	$get_q = mysql_fetch_assoc($sql);
	...
	$update_q = mysql_query("UPDATE loan_payment_details 
	...
	{//after perform the above sql
		...
		//update balance 
		$update_bal_q = mysql_query("UPDATE loan_payment_details 
										SET balance = '".$balance."', 
										branch_id = '".$_SESSION['login_branchid']."', 
										branch_name = '".$_SESSION['login_branch']."' 
										WHERE customer_loanid = '".$get_q['customer_loanid']."' 
		...
		{//after executiing the above sql
			...
			{//if there is no more balance
				...
				$nm = $get_q['month'] + 1;
				$insert_bal_q = mysql_query("INSERT INTO loan_payment_details SET 
												balance = '".$balance."', 
												customer_loanid = '".$get_q['customer_loanid']."', 
												month = '".$nm."'");
...
if($_POST['action'] == 'delete_loan')
{
	$id = $_POST['id'];
		...
		$sql = mysql_query("UPDATE customer_loanpackage SET loan_status = 'Deleted' WHERE id = '".$id."'");
		
		$_SESSION['msg'] = "<div class='success'>This loan's record has been successfully deleted from database.</div>";	
	}
else
if($_POST['action'] == 'delete_payout')
{
	$id = $_POST['id'];
		...
		$sql = mysql_query("UPDATE monthly_payment_record SET status = 'DELETED' WHERE id = '".$id."'");
		
		$_SESSION['msg'] = "<div class='success'>This payout's record has been successfully deleted from database.</div>";	
	}
else
if($_POST['action'] == 'bad_debt_payout')
{
	$id = $_POST['id'];
	
		$sql = mysql_query("UPDATE monthly_payment_record SET status = 'BAD DEBT' WHERE id = '".$id."'");
		
		$_SESSION['msg'] = "<div class='success'>This payout's record has been successfully recorded as Bad Debt.</div>";	
	}
}
?>