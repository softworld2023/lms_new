...
if(isset($_POST['search']))
{
	...
	//{
		...
		if($_POST['outdate'] != '')
		{
			...
			$cond .= " and pd.payment_date < '".$outdate."' and (lp.loan_status = 'Paid' OR lp.loan_status = 'Finished') AND (lp.payout_date <= '".$_SESSION['cdate']."' OR lp.payout_date = '0000-00-00')";
		}else
		{
       	...
       	<form action="" method="post">
        	<table>
				<tr>
                    ...
                    <td style="padding-right:30px"><input type="text" name="month" id="month" style="height:30px; width:100px" /></td>
	...
	if($_SESSION['cdate'] != '')
	{
		...
		if($scheme['receipt_type'] == 1)
		{
			...
			}else
			{
				...
				{
					//echo $get_q['loan_code']." - ".$outbalance."(".$_SESSION['cdate']." / ".$gouts['payment_date'].")";
				}
			}
		}else
		{//flexi receipt
			if($get_q['loan_status'] == 'Paid')
			{
				$rcheck_q = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND month_receipt = '".$_SESSION['pmonth']."'");
				$rcheck = mysql_fetch_assoc($rcheck_q);
				$rcheck_q2 = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND id > '".$rcheck['id']."' ORDER BY id DESC");
				$rcheck2 = mysql_fetch_assoc($rcheck_q2);
				...
				else
				{
					...
					$outst_q = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND month_receipt = '".$_SESSION['pmonth']."' ORDER BY month DESC");
					$row = mysql_num_rows($outst_q);
					$gouts = mysql_fetch_assoc($outst_q);
					...
					if($row > 1)
					{
						...
						if($gouts['payment_date'] == '0000-00-00')
						{
							...
							$outst_q2 = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND payment_date != '0000-00-00' AND month_receipt = '".$_SESSION['pmonth']."' ORDER BY month DESC");
							$getouts2 = mysql_fetch_assoc($outst_q2);
							if($getouts2['payment_date'] > $_SESSION['cdate'])
							{
								$outbalance = $getouts2['balance'];
							...
							{
								$outst_q2 = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND month_receipt = '".$_SESSION['pmonth']."' ORDER BY month DESC");
								$getouts2 = mysql_fetch_assoc($outst_q2);
								$outbalance = $getouts2['balance'];
							}	
						}else
			...
			}else
			{
				...
				$outbalance = 0;
				$outst_q = mysql_query("SELECT * FROM loan_payment_details WHERE customer_loanid = '".$get_q['id']."' AND balance != 0 AND month_receipt = '".$_SESSION['pmonth']."' ORDER BY month DESC");
				$gouts = mysql_fetch_assoc($outst_q);
