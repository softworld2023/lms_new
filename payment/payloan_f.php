...
<table width="1280" id="list_table">
    ...
    <tr>
		...
		<td>
			<div id="save_period" style="display:none"><a href="javascript:updatePeriod('<?php echo $get_q['loan_period'] ?>', '<?php echo $id; ?>')">
			<input type="button" name="saveperiod" id="saveperiod" value="" /></a></div>
			<input type="text" name="payday" id="payday" value="<?php echo $getacc['a_payday']; ?>" style="width:230px" readonly="readonly" />
	 </td>
    </tr>
    <tr>
        ...
        <td>
			<input type="text" name="loan_amount" value="<?php echo "RM ".number_format($get_q['loan_amount'], '2'); ?>" readonly="readonly" />&nbsp;
			<a href="payout.php?id=<?php echo $id; ?>" rel="shadowbox; width=500px; height=500px;"><img src="../img/customers/view-icon.png" /></a></td>
        <td style="padding:0px">Payment Date </td>
        <td><?php 
			if($get_q['loan_package'] != 'SKIM G') 
			{ ?>
				<input type="text" name="a_paymentdate" id="a_paymentdate" value="<?php echo $getacc['a_paymentdate']; ?>" readonly="readonly" /><?php 
			}
			else 
			{ ?>
				<input type="text" name="a_paymentdate" id="a_paymentdate" value="<?php echo date('d', strtotime($intrate['next_paymentdate'])); ?>" readonly="readonly" /><?php 
			} ?>
		</td>
    </tr> 
    <tr>
		...
		<td>
			<input type="text" name="intrate" id="intrate" style="width:40px" value="<?php echo $intrate['int_percent']; ?>%" />
			<input type="hidden" name="intratehide" id="intratehide" style="width:40px" value="<?php echo $intrate['int_percent']; ?>" />
		</td>
    ...
    <tr>
		<td style="padding:0px">Notes <?php 
			...
			{ ?>
				<a href="javascript:noteConfirm2('<?php echo $getacc['id']; ?>')" title="save notes"><img src="../img/document_save.png" /></a><?php 
			}
			else 
			{ ?>
				<a href="javascript:noteConfirm2('<?php echo $getacc['id']; ?>')" title="save notes"><img src="../img/document_save.png" /></a><?php 
		...
		<td rowspan="3"><textarea name="a_remarks" id="a_remarks" style="width:230px; height:90px"><?php echo $getacc['a_remarks']; ?><?php 
			if($getacc['update_date'] != '0000-00-00') 
			{ ?>
				<< Last Update By <?php echo $getacc['update_byname'];  
					echo " On ".date('d-m-Y', strtotime($getacc['update_date']))." >>"; 
			}  
			 ?></textarea>
		</td>	
		<td style="padding:0px">Loan Period <?php 
			if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen')) 
			{ ?>
				<a href="javascript:periodConfirm('<?php echo $id; ?>')" title="save loan period"><img src="../img/document_save.png" /></a><?php 
			} ?>
		</td>
		<td>
			<input type="text" name="period" id="period" value="<?php echo $get_q['loan_period']; ?>" style="width:50px" onkeyup="changePeriod(this.value,'<?php echo $get_q['loan_period']; ?>')" />months 
		</td>
		<td style="padding:0px">Remarks<?php 
			if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'staff' || $_SESSION['login_username'] == 'huizhen'))
			{ ?>
				<a href="javascript:remarksConfirm2('<?php echo $id; ?>')" title="save remarks"><img src="../img/document_save.png" /></a><?php 
			}
			else 
			{ ?>
				<a href="javascript:remarksConfirm('<?php echo $id; ?>')" title="save remarks"><img src="../img/document_save.png" /></a><?php 
			} ?>
		</td>
		<td rowspan="3">
			<textarea name="loanremarks" id="loanremarks" style="width:230px; height:90px"><?php echo $get_q['loan_remarks']; ?><?php
			if($get_q['update_loan_remarks_date'] != '0000-00-00')
			{?>
				<< Last Update By <?php echo $get_q['update_loan_remarks_by']; ?>
				<?php	echo " On ".date('d-m-Y', strtotime($get_q['update_loan_remarks_date']))." >>"; 
			} ?>
			</textarea>
		</td>	
    </tr>
...
<table width="1280" id="list_table">
	...
	$num_row = mysql_num_rows($loandetails_q);
...
<?php 
if($get_l['int_balance'] != 0 || $get_l['balance'] != 0) 
{
	// SKIM G words exist inside loan_package --- is SKIM G or is SKIM E
	if(strpos($get_q['loan_package'], 'SKIM G') !== false || $get_q['loan_package'] == 'SKIM E')
	{
		$a = $intrate['int_percent'];
		if ( $a != (int) ( $a ) ) 
		{
	...
	<tr <?php echo $style; ?>>
      	<td><?php echo $ctr; ?></td>
      	<td>
		<!--schedule date =  next payment date-->
			<?php 
			if($_SESSION['login_username'] != 'softworld' && $_SESSION['login_username'] != 'fong' && $_SESSION['login_username'] != 'huizhen' && $_SESSION['login_username'] != 'staff') 
			{
				echo date('d-m-Y', strtotime($get_l['next_paymentdate']));  
			}
			else 
			{ ?>
				<input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php
				// if still have balance
				if($get_l['next_paymentdate'] != '0000-00-00') 
				{
					echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); 
				} ?>" /> 
				<a href="javascript:edit_date('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save schedule date"><img src="../img/document_save.png" /></a><?php 
			} ?>
		</td>		
   	  	<td>
		<!-- payment date-->
			...
			{  ?><?php 
				if($style == '') 
				{?>
					<input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y', strtotime($get_l['next_paymentdate'])); ?>" />
					<input type="hidden" name="tdm" id="tdm" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" />
					<input type="text" name="payment_date" <?php 
					if($get_q['loan_package'] != 'SKIM G')  echo 'onblur="changeM(this.value)"'; ?> id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" /><?php 
				}
				else 
				{
					if($get_q['loan_package'] == 'SKIM G') 
					{?>
						<input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y', strtotime($get_l['next_paymentdate'])); ?>" />
						<input type="hidden" name="tdm" id="tdm" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" />
						<input type="text" name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" /><?php 
					}
					else
					{?>
						<input type="hidden" name="tdy" id="tdy" value="<?php echo date('Y'); ?>" />
						<input type="hidden" name="tdm" id="tdm" value="<?php echo date('m'); ?>" />
						<input type="text" name="payment_date" id="payment_date" <?php if($get_q['loan_package'] != 'SKIM G')  echo 'onblur="changeM(this.value)"'; ?> style="width:80px" value="<?php echo date('d-m-Y'); ?>" /><?php
					}
				} ?><?php
			}
			else 
			{
				...
				{?>
					...
					<a href="javascript:edit_pdate('<?php echo $get_l['id']; ?>', '<?php echo $ctr; ?>')" title="save payment date"><img src="../img/document_save.png" /></a><?php 
				}
				else
				{ 
					echo $get_l['payment_date'];
				}
			} ?>   
		</td>
   	  	<td>
		<!-- receipt no-->
			<?php echo $get_l['receipt_no']; ?>
		</td>
		<?php 
			if($ccm > $get_l['balance'])
			{
				$ccm = $get_l['balance'];
			}
			else
		...
		<!-- payment_received - ccm -->
			...
			{  ?><?php 
					...
					{?>
						...
						<input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccm, '2'); ?>" /><?php 
					}
					else 
					{ ?>
						<input type="hidden" name="id" id="id" value="<?php echo $get_l['id']; ?>" />
						<input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccm, '2'); ?>" />
						<?php 
					} ?><?php 
			}
			else 
		...
		<!-- payment received - interest-->
					...
					$print_q = mysql_query("SELECT SUM(payment_int) AS total_int_paid FROM loan_payment_details WHERE customer_loanid = '".$id."' AND next_paymentdate LIKE '%".$paymonth."%'");
					$print = mysql_fetch_assoc($print_q);
					
					$mbal = $ccmint - $print['total_int_paid'];
					//echo $ccmint."//".$print['total_int_paid']."//".$mbal;			
					...
					?>				
				<input type="hidden" name="new_receipt" id="new_receipt" style="width:120px" value="<?php echo $get_q['loan_code']; ?>"/><?php
				if($get_l['payment_date'] == '0000-00-00') 
				{  ?>
					<input type="hidden" name="intccm" id="intccm" value="<?php echo number_format($mbal, '0', '.', '');  ?>" />	<?php				
					if($style == '') 
					{?><?php 
						if($get_q['loan_package'] == 'SKIM HP')
						{ ?>
							<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckHP(this.value)" /><?php 
						}
						else if($get_q['loan_package'] == 'SKIM E')
						{ ?>
							<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckE(this.value)" /><?php 
						}
						else 
						{ ?>
							<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheck(this.value)" /><?php 
						} ?><?php 
					}
					else 
					{ ?><?php 
						...
						else if($get_q['loan_package'] == 'SKIM G')
						{ ?>
							<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheckG(this.value)" /><?php 
						} 
						else
						{ ?>				
							<input type="text" name="payment_int" id="payment_int" class="currency" style="width:100px" placeholder="RM <?php echo number_format($ccmint, '2'); ?>" onkeyup="monthlyCheck1(this.value)" /><?php 
						} ?><?php 
					} ?><?php 
				}
				// paid --- get record form database
				else 
				{
					echo "RM ".number_format($get_l['payment_int'], '2'); 
				} ?>            
		 </td>
         <td>
		 <!-- balance -->
				<?php echo "RM ".number_format($get_l['balance'], '2'); 
					if($num_row != 1)
					{
						$num_row--;
					}
					...
					{
						echo "<input type='hidden' name='bal_amount6' id='bal_amount6' value='".$get_l['balance']."'/>";
					}?><?php 	
					...
					{  ?><?php 
						...
						{?>
							...
							<input type="hidden" name="month_receipt" id="month_receipt" style="width:80px" value="<?php echo date('Y-m'); ?>"/><?php 
						}
						else 
						{ ?>
							<input type="hidden" name="new_loan" id="new_loan" style="width:80px" class="currency"/>
				...
				{
					$np_day = date('d', strtotime($intrate['next_paymentdate'])); 
				}
			...
			?>		
				<input type="hidden" name="hpnpd1" id="hpnpd1" value="<?php echo $newhpdate; ?>" /><?php 
				if($style == '') 
				{?>
					<input type="hidden" name="oripd" id="oripd" value="<?php echo $np_newdate; ?>" />
					<input type="hidden" name="next_paymentdate" id="next_paymentdate" style="width:80px" value="<?php echo $get_l['next_paymentdate']; ?>" readonly="readonly" /><?php 
				}
				else 
				{ ?>			
					<?php
					$mon1 = date('m', strtotime($get_l['next_paymentdate']));
        ...
        <td>
                <?php
				if($get_l['payment'] == '0' && $get_l['payment_int'] == '0' && $get_l['payment_date'] == '0000-00-00')
				// payment = payment received - ccm
				// payment_int = payment received - interest
				{ ?>
					<a href="javascript:payConfirm('<?php echo $get_q['loan_period']; ?>', '<?php echo $get_l['month']; ?>')" title="pay">
					<!--month = to check how many payment done -->
					<img src="../img/payment-received/payment-received.png" width="30" /></a>&nbsp;&nbsp;&nbsp;
					<?php
					if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'huizhen' || $_SESSION['login_username'] == 'staff')) 
					{?>
...
function payConfirm(period, month)
{
	...
	{
		$amount = document.getElementById('payment').value;	//payment received - ccm
		$ccm = document.getElementById('bal_amount6').value;	 // balance
		$date = document.getElementById('payment_date').value; //payment date
		$int_amount = document.getElementById('payment_int').value; // payment received - interest
		$nextdate = document.getElementById('next_paymentdate').value; //schedule date
		$id = document.getElementById('id').value; // $get_l['id']; -------- specific record
		$receipt = document.getElementById('new_receipt').value; //$get_q['loan_code']
		...
		$currmonth = month; //which record
		
		$check_bal = ($ccm*1) - ($amount*1);
			...
			if($check_bal <= 0)
			{
				if(($amount*1) != ($ccm*1))
				{
					$.confirm({
						...
						'message'	:  'Please enter the right payment amount! Balance = RM' + $ccm,
						'buttons'	: {
							'Ok'	: {
							'class'	: 'blue',
							'action': function(){}	// Nothing to do in this case. You can as well omit the action property.
...
function monthlyCheck(a)
{
	...
	$monthly = $monthly1 * 1; //remaining
	$m1 = document.getElementById('tdm').value; //today month
	$y1 = document.getElementById('tdy').value; //today year
	$d = document.getElementById('a_paymentdate').value; //day
	$a = a; // interest payment
	$m = $m1 * 1;
	$y = $y1 * 1;
...
function monthlyCheckE(a)
{
	...
	$m = $m1 * 1;
	$y = $y1 * 1;
	$payment1 = document.getElementById('payment').value;
	$payment = $payment1 * 1;
	
	if($payment != 0)
	{
		$intrate1 = document.getElementById('intratehide').value;
		$intrate = ($intrate1 * 1)/100;
		$paymentint = $payment * $intrate;
		if($a > ($monthly-$paymentint) || $a == ($monthly-$paymentint))
		{
			$m = $m + 1;
	...
	else if($payment == 0 && ($a > $monthly || $a == $monthly))
	{	
		$m = $m + 1;
...
function deletepayConfirm(loanid, id, date,amount)
{
	$id = id;
	$loanid = loanid;
	$amount = amount;
