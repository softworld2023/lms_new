...
$id = $_GET['id'];//id is probably get from the payloan's link!
...
//$babi = mysql_fetch_assoc($loandetails_q);
//echo $finalloan['customer_loanid'];
...
<table width="1280" id="list_table">
    ...
    <tr>
      <td style="padding:0px">Notes <?php if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'huizhen')) { ?>
...
<table width="1280" id="list_table">
	...
	<tr <?php echo $style;?>>
      	<td><?php echo $ctr; ?></td>
      	<td><?php if($_SESSION['login_username'] != 'softworld' && $_SESSION['login_username'] != 'fong' && $_SESSION['login_username'] != 'huizhen') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } else { ?>
			<input type="text" name="edit_npd_<?php echo $ctr; ?>" id="edit_npd_<?php echo $ctr; ?>" style="width:80px" value="<?php if($get_l['next_paymentdate'] != '0000-00-00') { echo date('d-m-Y', strtotime($get_l['next_paymentdate'])); } ?>" />
		...
		<input type="hidden" name="schedule" id="schedule" value="<?php echo date('Y', strtotime($get_l['next_paymentdate'])); ?>" />
			<?php if($style == '') {?>
            <input type="text" name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" />
            <?php } else { ?>
			<input type="text"  name="payment_date" id="payment_date" style="width:80px" value="<?php echo date('d-m-Y'); ?>" />
            <?php } ?>
            <?php } else { if($_SESSION['login_level'] == 'Boss' || ($_SESSION['login_username'] == 'softworld' || $_SESSION['login_username'] == 'fong' || $_SESSION['login_username'] == 'zoey'|| $_SESSION['login_username'] == 'huizhen' )) { ?>
			...
			<?php //if($latectr == $latecount) { ?>
				...
				<input type="text" name="payment" id="payment" class="currency" style="width:100px" placeholder="RM" />
		...
		<!-- NEW RECEIPT -->
		<td>
			<?php if($get_l['payment_date'] == '0000-00-00') {// invalid date ?>
			
			<?php 	if($style == '') {?>
			<input type="text" name="new_receipt" id="new_receipt" style="width:80px" 
				value="<?php echo $get_q['loan_code']; ?>" onblur="checkReceipt2(this.value, '<?php echo $id; ?>')" />
			...
			<?php //if($latectr == $latecount) { ?>
			<!-- This all are just displaying the data fetched from database -->
			<input type="text" name="new_receipt" id="new_receipt" style="width:80px" 
				value="<?php echo $get_q['loan_code']; ?>" onblur="checkReceipt2(this.value, '<?php echo $id; ?>')" />
			...
			<?php }//end of  [if($get_l['payment_date'] == '0000-00-00')]
			
				else { /*echo $get_l['receipt_no'];*/
				$newmonth = $get_l['month'] + 1;
				$newloan_q = mysql_query("SELECT * FROM loan_payment_details 
											WHERE customer_loanid = '".$get_l['customer_loanid']."' 
											AND month = '".$newmonth."'");
				$newloan = mysql_fetch_assoc($newloan_q);
				//here it doesn't shows the replacement of data lehhhh.... =_=!!!
				//alright I didn't find the replacement of data here zzzz
		...
		<td>
            ...
            <td><input type="hidden" name="pnpdate" id="pnpdate" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" />
				<?php if($get_l['payment_date'] == '0000-00-00') {  ?><input type="hidden" name="precm" id="precm" value="<?php echo date('m', strtotime($get_l['next_paymentdate'])); ?>" />
				<?php if($style == '') {?>
				<input type="text" name="month_receipt" id="month_receipt" style="width:80px" onfocus="nextdate(this.value)" onblur="checkReceipt(this.value, '<?php echo $id; ?>')"/>
				<?php } else { ?>
				<?php if($latectr == $latecount) { ?>
				<input type="text" name="month_receipt" id="month_receipt" style="width:80px" onfocus="nextdate(this.value)" onblur="checkReceipt(this.value, '<?php echo $id; ?>')"/>
				<?php } else { 
				...
				if($np_newdate == date('d-m-Y', strtotime($get_l['next_paymentdate'])))
				{
					//$np_month = $np_month + 1;
				...
				$new_month = $get_l['month'] + 1; 
					$npd_q = mysql_query("SELECT * FROM loan_payment_details 
											WHERE customer_loanid = '".$id."' 
											AND month = '".$new_month."'");	
            ...
            <td style="padding:0px">
				...
				<input type="hidden" name="prm" id="prm" value="<?php echo $get_l['month_receipt']; ?>" />
                ...
                <a href="javascript:payConfirm('<?php echo $get_q['loan_period']; ?>', '<?php echo $get_l['month']; ?>','<?php echo $get_l['balance']; ?>')" title="pay"><img src="../img/payment-received/payment-received.png" width="30" /></a>&nbsp;&nbsp;&nbsp;<?php
				...
				{?>
				<a href="adjustflexi.php?id=<?php echo $get_l['id']; ?>&p=<?php echo $get_q['loan_period']; ?>" rel="shadowbox; width=800px; height=380px" title="adjust"><img src="../img/customers/edit-icon.png" /></a>
        	...
        	?>
					...
					?>
						<?php 
							$checkrow_q = mysql_query("SELECT * FROM loan_payment_details 
														WHERE id > '".$get_l['id']."'
														AND customer_loanid = '".$id."'"); 
...
function payConfirm(period, month, balance)
{
	...
	else
	{
		...
		/*	
		if(($amount != '' || $int_amount != '' ) && $date != '' && $mr != '' && $nextdate != '' && $receipt != '' && $pamount == $balance)
		*/
...
function checkReceipt(str, loanid)
{
	...
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
		//this create a new object called XMLHttpRequest
	  ...
	  {// code for IE6, IE5
		//this is the old way to create this object
