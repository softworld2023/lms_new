...
$pmonth1 = strtotime($month .' -1 months');
$pmonth = date('Y-m', $pmonth1);
...
if($get_p['type'] == 'Flexi Loan')
{
	...
	{
		//$cf_q = mysql_query("SELECT SUM(balance) AS cfamount FROM loan_payment_details WHERE package_id = '".$package_id."' AND month_receipt = '".$month."'");
		//$get_cf = mysql_fetch_assoc($cf_q);
		
		$cfamt = 0;
		$prev_receipt = '';
		$cf_q = mysql_query("SELECT * FROM loan_payment_details WHERE package_id = '".$package_id."' AND month_receipt = '".$month."' ORDER BY receipt_no");
		while($get_cf = mysql_fetch_assoc($cf_q))
		{
			if($prev_receipt != $get_cf['receipt_no'])
			{
				$cfamt += $get_cf['balance'];
			}
			$prev_receipt = $get_cf['receipt_no'];
		}
		
		//$prev_cf = mysql_query("SELECT SUM(balance) AS pcfamount FROM loan_payment_details WHERE package_id = '".$package_id."' AND month_receipt = '".$pmonth."'");
		//$get_pcf = mysql_fetch_assoc($prev_cf);
...
$cf_datemonth = date('Y-m', strtotime($cf_date));

if($cf_datemonth == $month)
{
	$cf = $get_p['bal2_initial_amount'];
}
$nmth = $month.'-01'; 
...
if($get_p['type'] == 'Flexi Loan')
{
	if($get_p['receipt_type'] == '2')
	{
		...
		$prevbal_q = mysql_query("SELECT SUM(loan) AS amtloan, SUM(received) AS amtrec, SUM(commission) AS amtcom, SUM(interest) AS amtint FROM balance_rec WHERE package_id = '".$package_id."' AND month_receipt < '".$month."' AND bal_date < '".$newmth."'");
		...
		<table width="1280" id="list_table">
			...
			while($get_l = mysql_fetch_assoc($loan_q)){
				...
				if($d != $prev_d)
				{
					$sum_q = mysql_query("SELECT SUM(loan) AS sumLoan, SUM(received) AS sumRec, SUM(commission) AS sumCom, SUM(interest) AS sumInt FROM balance_rec WHERE month_receipt = '".$month."' AND bal_date LIKE '%".$d."%' AND package_id = '".$package_id."' ORDER BY bal_date ASC");
					$get_sum = mysql_fetch_assoc($sum_q);		
					
					
					if($get_sum['sumRec'] != 0 || $get_sum['sumCom'] != 0 || $get_sum['sumInt'] != 0)
					{
						$in = $get_sum['sumRec'] + $get_sum['sumCom'] + $get_sum['sumInt'];
						$tot = $tot + $in;
...
<?php
	...
	<table width="1280" id="list_table">
		...
		while($get_l = mysql_fetch_assoc($loan_q)){
			...
			if($d != $prev_d)
			{
				$sum_q = mysql_query("SELECT SUM(loan) AS sumLoan, SUM(received) AS sumRec, SUM(interest2) AS sumInt2, SUM(commission) AS sumCom, SUM(interest) AS sumInt FROM balance_rec WHERE bal_date LIKE '%".$d."%' AND package_id = '".$package_id."' ORDER BY bal_date ASC");
				$get_sum = mysql_fetch_assoc($sum_q);		
