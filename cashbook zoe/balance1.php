...
$loan_q = mysql_query("SELECT * FROM balance_transaction WHERE package_id = '".$package_id."' GROUP BY CONCAT(YEAR(date), '-', MONTH(date)) ORDER BY date DESC");
...
<table width="1280" id="list_table">
    ...
    while($get_l = mysql_fetch_assoc($loan_q)){
		...
		$prevbal_q = mysql_query("SELECT SUM(loan) AS amtloan, SUM(received) AS amtrec FROM balance_transaction WHERE package_id = '".$package_id."' AND date LIKE '%".$month_cb."%'");
