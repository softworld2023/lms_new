...
if($_POST['action'] == 'payloanf2') //flexi tapi resit guna code yang sama
{
	...
	$sql = mysql_query("SELECT * FROM loan_payment_details WHERE nric = '".$nric."' AND month = '".$month."'",db2);
	$get_q = mysql_fetch_assoc($sql);
	
	$update_q = mysql_query("UPDATE loan_payment_details SET cust_id='".$id."' payment = '".$amount."', payment_int = '".$int_amount."', payment_date = '".$date."', receipt_no = '".$receipt_no."', month_receipt = '".$month_receipt."', staff_id = '".$_SESSION['taplogin_id']."', staff_name = '".$_SESSION['login_name']."', created_date = now() WHERE nric = '".$nric."' AND month = '".$month."'",db2);
	...
	//{
		...
		if($loan_balance > 0)
		{
			...
			if($amount == 0)
			{
				...
				$nxmr_q = mysql_query("SELECT * FROM loan_payment_details WHERE nric = '".$nric."' AND month > '".$month."'",db2);							
				...
				if(!$nxmr)
				{
					...
					$nm = $get_q['month'] + 1;
					$insert_lp_q = mysql_query("INSERT INTO loan_payment_details SET nric = '".$nric."', customer_loanid = '".$customer_loanid."', month = '".$nm."', int_percent = '".$get_q['int_percent']."', balance = '".$loan_balance."',next_paymentdate = '".$next_paymentdate."', package_id = '".$get_q['package_id']."', receipt_no = '".$receipt_no."', month_receipt = '".$month_receipt."', branch_id = '".$_SESSION['login_branchid']."', branch_name = '".$_SESSION['login_branch']."'");
				}
				else
				{
				
					$insert_lp_q = mysql_query("UPDATE loan_payment_details SET balance = '".$loan_balance."' WHERE month > '".$month."' AND customer_loanid = '".$get_q['customer_loanid']."' AND branch_id = '".$_SESSION['login_branchid']."'");
				}
			...
			}else
			{
				...
				{
					$insert_lp_q = mysql_query("UPDATE loan_payment_details SET balance = '".$loan_balance."' WHERE month > '".$month."' AND customer_loanid = '".$get_q['customer_loanid']."' AND branch_id = '".$_SESSION['login_branchid']."'",db2);
				}
