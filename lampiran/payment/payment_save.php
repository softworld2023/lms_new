...
if(isset($_POST['save']))
{
	...
	$customer_id = strtoupper($_POST['customercode']);
	$customer_name = strtoupper($_POST['name']);
	$nric = $_POST['nric'];
	$phone_no =  $_POST['contact'];
	$notes = strtoupper($_POST['aremarks']);
	...
	$bankacc = strtoupper($_POST['bankacc']);
	$paydate = $_POST['payday'];
	$paymentdate = $_POST['a_paymentdate'];
	$salary = $_POST['salary'];
	$loanremarks = strtoupper($_POST['loanremarks']);
	...
	{
		$update = mysql_query("UPDATE customer_details SET phone_no = '".$phone_no."', 
												  notes= '".$notes."', 
												  bankacc= '".$bankacc."', 
												  remark= '".$loanremarks."', 
	...
	if($loan_amount!=0){
		...
		if($insert || $insert1 || $insert2)
		{
			...
			echo "<script>window.location='../payment/view_details.php?ic=".$nric."&branch_id=".$branch_id."&loanid=".$loan_code."'</script>";
		}
