...
if($_POST['action'] == 'delete_payfixed2')//delete for payloan.php
{
	$id = $_POST['id'];
	$nric = $_POST['nric'];
	//$package_id = $_POST['package_id'];

	$lpd_q = mysql_query("SELECT * FROM loan_payment_details WHERE nric = '".$nric."' AND id = '".$id."'");	
	$lpd = mysql_fetch_assoc($lpd_q);
	
	//next record
	//$lpd2_q = mysql_query("SELECT * FROM loan_payment_details WHERE nric = '".$nric."' AND id > '".$id."'");	
	...
	{
		//delete next record 
		$delete_nrcd = mysql_query("DELETE FROM loan_payment_details WHERE nric = '".$nric."' AND id = '".$id."'",$db2);
	}
	if($lpd)
				{
					$msg .= 'Payment has been successfully delete from the record.<br>';
