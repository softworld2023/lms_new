...
<?php
	...
	// if(isset($_POST)){
		...
		//  Customer Details
		// =============================
		$bis = isset($_POST['bis']) ? addslashes(strtoupper($_POST['bis'])) : '';
		$name = isset($_POST['name']) ? addslashes(strtoupper($_POST['name'])) : '';
		$title = isset($_POST['title']) ? addslashes(strtoupper($_POST['title'])) : '';
		...
		$nationality = isset($_POST['nationality']) ? addslashes($_POST['nationality']) : '';
		$old_ic = isset($_POST['old_ic']) ? addslashes($_POST['old_ic']) : '';
		$nric = isset($_POST['nric']) ? addslashes($_POST['nric']) : '';
		$gender = isset($_POST['gender']) ? addslashes($_POST['gender']) : '';
		$race = isset($_POST['race']) ? addslashes($_POST['race']) : '';
		$source = isset($_POST['source']) ? addslashes(strtoupper($_POST['source'])) : '';
		$marital_status = isset($_POST['marital_status']) ? addslashes($_POST['marital_status']) : '';
		$no_dependents = isset($_POST['no_dependents']) ? $_POST['no_dependents'] : '';

		$dependent1 = isset($_POST['dependent1']) ? $_POST['dependent1'] : '';
		$dependent2 = isset($_POST['dependent2']) ? $_POST['dependent2'] : '';
		$dependent3 = isset($_POST['dependent3']) ? $_POST['dependent3'] : '';
		$dependent4 = isset($_POST['dependent4']) ? $_POST['dependent4'] : '';
		$dependent5 = isset($_POST['dependent5']) ? $_POST['dependent5'] : '';
		$dependent6 = isset($_POST['dependent6']) ? $_POST['dependent6'] : '';

		$dependent6 = isset($_POST['dependent6']) ? $_POST['dependent6'] : '';
		$academic_qualification = isset($_POST['academic_qualification']) ? addslashes(strtoupper($_POST['academic_qualification'])) : '';
		$mother_name = isset($_POST['mother_name']) ? addslashes(strtoupper($_POST['mother_name'])) : '';
		$others_ic = isset($_POST['others_ic']) ? $_POST['others_ic'] : '';
		$others_ic2 = isset($_POST['others_ic2']) ? $_POST['others_ic2'] : '';
		...
		//  Customer Address
		// =============================
		$address1 = isset($_POST['address1']) ? addslashes(strtoupper($_POST['address1'])) : '';
		$address2 = isset($_POST['address2']) ? addslashes(strtoupper($_POST['address2'])) : '';
		$address3 = isset($_POST['address3']) ? addslashes(strtoupper($_POST['address3'])) : '';
		$postcode = isset($_POST['postcode']) ? $_POST['postcode'] : '';
		$city = isset($_POST['city']) ? addslashes(strtoupper($_POST['city'])) : '';
		$state = isset($_POST['state']) ? addslashes(strtoupper($_POST['state'])) : '';
		$residence = isset($_POST['residence']) ? addslashes($_POST['residence']) : '';
		$year_stay = isset($_POST['year_stay']) ? $_POST['year_stay'] : '';
		$home_contact = isset($_POST['home_contact']) ? $_POST['home_contact'] : '';
		$mobile_contact = isset($_POST['mobile_contact']) ? $_POST['mobile_contact'] : '';

		$m_address1 = isset($_POST['m_address1']) ? addslashes(strtoupper($_POST['m_address1'])) : '';
		$m_address2 = isset($_POST['m_address2']) ? addslashes(strtoupper($_POST['m_address2'])) : '';
		$m_address3 = isset($_POST['m_address3']) ? addslashes(strtoupper($_POST['m_address3'])) : '';
		$m_postcode = isset($_POST['m_postcode']) ? $_POST['m_postcode'] : '';
		$m_city = isset($_POST['m_city']) ? addslashes(strtoupper($_POST['m_city'])) : '';
		$m_state = isset($_POST['m_state']) ? addslashes(strtoupper($_POST['m_state'])) : '';
		$m_residence = isset($_POST['m_residence']) ? addslashes($_POST['m_residence']) : '';
		$m_year_stay = isset($_POST['m_year_stay']) ? $_POST['m_year_stay'] : '';
		$month_stay = isset($_POST['month_stay']) ? $_POST['month_stay'] : '';
		$m_month_stay = isset($_POST['m_month_stay']) ? $_POST['m_month_stay'] : '';
	...
	if (!empty($_POST['signature_data'])) {
		$raw_data = $_POST['signature_data'];

		// Remove base64 header if present
		$raw_data = str_replace('data:image/png;base64,', '', $raw_data);
		$raw_data = str_replace(' ', '+', $raw_data);
		$image_data = base64_decode($raw_data);

		// Folder to store signature files
		$signature_folder = '../files/customer_signatures/';
		if (!file_exists($signature_folder)) {
			mkdir($signature_folder, 0777, true);
		}

		// Create unique filename
		$signature_filename = 'signature_' . time() . '_' . $customer_id . '.png';
		$signature_path_full = $signature_folder . $signature_filename;

		// echo "<pre>";
		// echo "Customer ID: $customer_id\n";
		// echo "Signature Folder: $signature_folder\n";
		// echo "Signature Path: $signature_path_full\n";
		// echo "Decoded Image Size: " . strlen($image_data) . " bytes\n";
		// echo "Folder Exists? " . (file_exists($signature_folder) ? "Yes" : "No") . "\n";
		// echo "Folder Writable? " . (is_writable($signature_folder) ? "Yes" : "No") . "\n";
		// exit;

		// Save the decoded image data
		if (file_put_contents($signature_path_full, $image_data) === false) {
			die("❌ Failed to save signature image to: $signature_path_full");
		}

		// Store relative path for database
		$signature_path = mysql_real_escape_string('files/customer_signatures/' . $signature_filename);
	}
	...
	$update_details_q = mysql_query("UPDATE customer_details 
									...
									cust_apply_date = '".$cust_apply_date."'
	...
	// First check if the customer already has an account
	$check_q = mysql_query("SELECT id FROM customer_account WHERE customer_id = '".$customer_id."' ");
	if (mysql_num_rows($check_q) > 0) {
		// =============================
		//  UPDATE existing record
		// =============================
		$update_q = mysql_query("
			UPDATE customer_account SET 
				a_bankbranch = '".$a_bankbranch."', 
				a_payday = '".$a_payday."', 
				a_paymentdate = '".$a_paymentdate."', 
				a_name = '".$a_name."', 
				a_nric = '".$a_nric."', 
				a_date = '".$a_date."', 
				a_pinno = '".$a_pinno."', 
				...
				a_bankaccno = '".$a_bankaccno."', 
				transfer_accountbank = '".$transfer_accountbank."', 
				transfer_accountno = '".$transfer_accountno."', 
				transfer_accountholder = '".$transfer_accountholder."', 
		...
		//  INSERT new record
		// =============================
		$insert_q = mysql_query("
			INSERT INTO customer_account SET 
	...
	if($loan_amount!='' && $loan_period!='' && $loan_pokok!=''){
		$check_exist_loanpackage = mysql_query("SELECT * FROM customer_loanpackage_draft WHERE customer_id = '".$customer_id."' AND loan_code = '".$loan_code."'");
		$check_exist_loanpackage_num = mysql_num_rows($check_exist_loanpackage);
		
		if($check_exist_loanpackage_num > 0){
			$update_loanpackage = mysql_query("UPDATE customer_loanpackage_draft SET customer_id = '".$customer_id."' WHERE customer_id = '".$customer_id."' AND loan_code = '".$loan_code."'");
		} else {
	...
	if($_POST['payout_amount']!='' || $_POST['payout_amount']!='0'){
		...
		$check_exist_record_q = mysql_query("SELECT * FROM monthly_payment_record_draft WHERE customercode = '".$customer_id."' AND status = 'PAID'");
		$check_exist_record = mysql_num_rows($check_exist_record_q);
			
			if($check_exist_record > 0){
				$update_q = mysql_query("UPDATE monthly_payment_record_draft SET customer_id = '".$customer_id."' WHERE customercode = '".$customer_id."' AND status = 'PAID' AND loan_code = '".$loan_code_monthly."'");
			} else {
				//insert skim kutu
